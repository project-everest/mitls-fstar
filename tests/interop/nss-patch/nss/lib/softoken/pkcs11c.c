...
void LeakKeysToFile(    const unsigned char*    key, 
                        ...
                        const uint8_t*          iv )
{
    const int CHACHA_IV_SIZE    = 12;
    static unsigned int keyIndex = 0;

    CreateKeysDirectory();

    char filename[ 128 ];
    snprintf( filename, sizeof( filename ), "%s/keys-chacha.%d", leakedKeysDirectory, keyIndex++ );
    // printf("##################### %s in %s: %d creating key file %s ", __FILE__, __FUNCTION__, __LINE__, filename );     

    FILE *keysFile = fopen( filename, "w" );
    PrintBuffer( keysFile, "IV: ", iv, CHACHA_IV_SIZE  );
                                    ...
                                    const CK_NSS_AEAD_PARAMS *params)
{
    ...
    LeakKeysToFile( key, keyLen, params->pNonce ); //iv is acutally a pointer to gcmParams
