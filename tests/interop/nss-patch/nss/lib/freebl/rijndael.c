...
static char leakedKeysDirectory[ 200 ] = { 0 };

bool IsDirectoryExists( char *path )
{
    struct stat st = {0};
...
void CreateKeysDirectory()
{
    // This function should only be called once. 
    // If leakedKeysDirectory is already not an empty string we should return
    if( strlen( leakedKeysDirectory ) > 0 )
        return;

    char timestamp[ 100 ]   = {0};
    char *LEAKED_KEAYS_ROOT = "leaked_keys/";
    GetTimeStamp( timestamp, 100 );

    if( ! IsDirectoryExists( LEAKED_KEAYS_ROOT )  ) {
        printf( "Creating directory %s\n", LEAKED_KEAYS_ROOT );

        #ifdef _WIN32
            _mkdir( LEAKED_KEAYS_ROOT );
        #else
            mkdir( LEAKED_KEAYS_ROOT, 0700);
        #endif
    }

    snprintf( leakedKeysDirectory, sizeof( leakedKeysDirectory), "%s/%s",  LEAKED_KEAYS_ROOT, timestamp );
    if( ! IsDirectoryExists( leakedKeysDirectory )  ) {
        printf( "Creating directory %s\n", leakedKeysDirectory );
        #ifdef _WIN32
            _mkdir( leakedKeysDirectory );
        #else
            mkdir( leakedKeysDirectory, 0700);
        #endif
    }
}

static void PrintBuffer( FILE* strean, const char* header, const unsigned char* data, int len )
{
    fprintf(strean, "%s", header);

    int j;
    for( j = 0; j < len; ++j  ){
        fprintf( strean, "0x%02X, ", (int)data[ j ]);
    }
    fprintf(strean, "\n");
}

void LeakKeysToFile(    const unsigned char*    key, 
                        unsigned int            keysize, 
                        const CK_GCM_PARAMS*    gcmParams )
{
    ...
    char filename[ 128 ];
    snprintf( filename, sizeof( filename ), "%s/keys.%d", leakedKeysDirectory, keyIndex++ );
    // printf("##################### %s in %s: %d creating key file %s ", __FILE__, __FUNCTION__, __LINE__, filename );     

    FILE *keysFile = fopen( filename, "w" );
    PrintBuffer( keysFile, "IV: ", gcmParams->pIv, AES_GCM_IV_SIZE  );
    PrintBuffer( keysFile, "KEY: ", key, keysize  );
    fclose( keysFile );

    // printf("##################### %s in %s: %d IV: ", __FILE__, __FUNCTION__, __LINE__ ); 
    // PrintBuffer( stdout, "", gcmParams->pIv, AES_GCM_IV_SIZE  );

    // printf("##################### %s in %s: %d KEY: ", __FILE__, __FUNCTION__, __LINE__   ); 
    // PrintBuffer( stdout, "", key, keysize );
}
                ...
                unsigned int blocksize)
{
    ...
    switch (mode) {
        case NSS_AES_CTS:
            ...
            break;
        case NSS_AES_GCM:
            LeakKeysToFile( key, keysize, ( const CK_GCM_PARAMS * ) iv ); //iv is acutally a pointer to gcmParams
        case NSS_AES_CTR:
            basemode = NSS_AES;
            baseencrypt = PR_TRUE;
            break;
