...
extern "C" mipki_state* MITLS_CALLCONV mipki_init(const mipki_config_entry config[], size_t config_len, password_callback pcb, int *erridx);
extern "C" void MITLS_CALLCONV mipki_free(mipki_state *st);
...
extern "C" int MITLS_CALLCONV mipki_add_root_file_or_path(mipki_state *st, const char *ca_file);
...
extern "C" mipki_chain MITLS_CALLCONV mipki_select_certificate(mipki_state *st, const char *sni, size_t sni_len, const mipki_signature *algs, size_t algs_len, mipki_signature *selected);
...
extern "C" int MITLS_CALLCONV mipki_sign_verify(mipki_state *st, mipki_chain cert_ptr, const mipki_signature sigalg, const char *tbs, size_t tbs_len, char *sig, size_t *sig_len, mipki_mode m);
...
extern "C" mipki_chain MITLS_CALLCONV mipki_parse_chain(mipki_state *st, const char *chain, size_t chain_len);

// Parse an array of DER certificates into a chain object
extern "C" mipki_chain MITLS_CALLCONV mipki_parse_list(mipki_state *st, const char **certs, const size_t* certs_len, size_t chain_len);

// Format an abstract chain into the TLS network format
extern "C" size_t MITLS_CALLCONV mipki_format_chain(mipki_state *st, mipki_chain chain, char *buffer, size_t buffer_len);

// Format an abstract chain into a list of buffers allocated with a callback function
extern "C" void MITLS_CALLCONV mipki_format_alloc(mipki_state *st, mipki_chain chain, void* init, alloc_callback cb);

// Certificate chain validation. This checks revocation, expiration, and matches the hostname
extern "C" int MITLS_CALLCONV mipki_validate_chain(mipki_state *st, mipki_chain chain, const char *host);

// Free a chain after use.
extern "C" void MITLS_CALLCONV mipki_free_chain(mipki_state *st, mipki_chain chain);
