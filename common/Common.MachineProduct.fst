...
type machine_iface
  ...
  = {
  csm     : SM.state_machine cst wire cloc lout;
  ssm     : SM.state_machine sst wire sloc lout;
...
let mp_client_send
  ...
  (exists (local:cloc) (c':cst) (out:SM.step_output wire lout) (p:payload).
     i.csm.SM.sm_step a.client (SM.LocalEvent local) c' out /\
...
let mp_server_send
  ...
  (exists (local:sloc) (s':sst) (out:SM.step_output wire lout) (p:payload).
     i.ssm.SM.sm_step a.server (SM.LocalEvent local) s' out /\
     Cons? out.SM.so_wire_outputs /\
...
let mp_deliver_to_client
  ...
  (exists (p:payload) (w:wire) (c':cst) (out:SM.step_output wire lout).
     ...
     i.carries p w /\
     i.csm.SM.sm_step a.client (SM.WireEvent w) c' out /\
...
let mp_deliver_to_server
  ...
  (exists (p:payload) (w:wire) (s':sst) (out:SM.step_output wire lout).
     ...
     i.carries p w /\
     i.ssm.SM.sm_step a.server (SM.WireEvent w) s' out /\
...
(** The initial system: both endpoints at their state machines' initial states,
    nothing in flight. **)
let initial_sys
  (#cst #sst #payload #wire #cloc #sloc #lout:Type0)
  (i:machine_iface cst sst payload wire cloc sloc lout)
  : sys cst sst payload = {
  client  = i.csm.SM.sm_initial_state;
  server  = i.ssm.SM.sm_initial_state;
  channel = Quiet;
}
...
(** Each endpoint only ever moves by its OWN state machine.  This is the formal
    ...
    the client component, it is a single `csm.sm_step`. **)
  ...
  : Lemma
      ...
      (ensures
        (exists (ev:SM.event wire cloc) (out:SM.step_output wire lout).
           i.csm.SM.sm_step a.client ev b.client out))
  = let goal =
      (exists (ev:SM.event wire cloc) (out:SM.step_output wire lout).
         i.csm.SM.sm_step a.client ev b.client out) in
    eliminate
      (mp_client_send i a b) \/ (mp_deliver_to_client i a b \/ mp_client_local i a b)
    returns goal
    with _hsend. begin
      ...
      returns goal
      with _pf.
        assert (i.csm.SM.sm_step a.client (SM.LocalEvent local) b.client out)
    end
    and _hrest. begin
      ...
      with _hdel. begin
        ...
        returns goal
        with _pf.
          assert (i.csm.SM.sm_step a.client (SM.WireEvent w) b.client out)
      end
      and _hloc. begin
  ...
  : Lemma
      ...
      (ensures
        (exists (ev:SM.event wire sloc) (out:SM.step_output wire lout).
           i.ssm.SM.sm_step a.server ev b.server out))
  = let goal =
      (exists (ev:SM.event wire sloc) (out:SM.step_output wire lout).
         i.ssm.SM.sm_step a.server ev b.server out) in
    eliminate
    ...
    with _hsend. begin
      ...
      returns goal
      with _pf.
        assert (i.ssm.SM.sm_step a.server (SM.LocalEvent local) b.server out)
    end
    and _hrest. begin
      eliminate
      ...
      with _hdel. begin
        ...
        returns goal
        with _pf.
          assert (i.ssm.SM.sm_step a.server (SM.WireEvent w) b.server out)
      end
      and _hrest2. begin
...
let lemma_client_valid_preserved
  ...
  (a b:sys cst sst payload)
  : Lemma
      (requires machine_step i a b /\ SM.valid_state i.csm a.client)
      (ensures SM.valid_state i.csm b.client)
  = lemma_client_moves_or_fixed i a b;
    eliminate
      (b.client == a.client) \/
      (exists (ev:SM.event wire cloc) (out:SM.step_output wire lout).
         i.csm.SM.sm_step a.client ev b.client out)
    returns SM.valid_state i.csm b.client
    with _hfix. ()
    and  _hstep.
      eliminate exists (ev:SM.event wire cloc) (out:SM.step_output wire lout).
        i.csm.SM.sm_step a.client ev b.client out
      returns SM.valid_state i.csm b.client
      with _pf. SM.lemma_valid_state_after_step i.csm a.client ev b.client out
...
let lemma_server_valid_preserved
  ...
  (a b:sys cst sst payload)
  : Lemma
      (requires machine_step i a b /\ SM.valid_state i.ssm a.server)
      (ensures SM.valid_state i.ssm b.server)
  = lemma_server_moves_or_fixed i a b;
    eliminate
      (b.server == a.server) \/
      (exists (ev:SM.event wire sloc) (out:SM.step_output wire lout).
         i.ssm.SM.sm_step a.server ev b.server out)
    returns SM.valid_state i.ssm b.server
    with _hfix. ()
    and  _hstep.
      eliminate exists (ev:SM.event wire sloc) (out:SM.step_output wire lout).
        i.ssm.SM.sm_step a.server ev b.server out
      returns SM.valid_state i.ssm b.server
      with _pf. SM.lemma_valid_state_after_step i.ssm a.server ev b.server out
