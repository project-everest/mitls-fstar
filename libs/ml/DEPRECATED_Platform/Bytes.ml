...
let equalBytes (b1:bytes) (b2:bytes) =
    if length b1 = length b2 then
       let cb1 = cbytes b1 in
       let cb2 = cbytes b2 in
       let ok = ref true in
...
let iutf8 (x:bytes) : string = cbytes x (* TODO: use Camomile *)
let iutf8_opt (x:bytes) : string = Some (cbytes x)
