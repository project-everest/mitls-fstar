...
// be called before or after MiTLS_FFI_mitls_init().
typedef void (MITLS_CALLCONV *pfn_mitls_trace_callback)(const char *msg);
extern void MITLS_CALLCONV MiTLS_FFI_mitls_set_trace_callback(pfn_mitls_trace_callback cb);

// Perform one-time initialization
extern int MITLS_CALLCONV MiTLS_FFI_mitls_init(void);
...
extern int MITLS_CALLCONV MiTLS_FFI_mitls_set_ticket_key(const char *alg, const unsigned char *ticketkey, size_t klen);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_set_sealing_key(const char *alg, const unsigned char *sealingkey, size_t klen);

// Perform one-time termination
extern void MITLS_CALLCONV MiTLS_FFI_mitls_cleanup(void);

// Configure miTLS ahead of connecting
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure(/* out */ mitls_state **state, const char *tls_version, const char *host_name);

// Configure a ticket to resume (client only). Can be called more than once to offer multiple 1.3 PSK
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_ticket(mitls_state *state, const mitls_ticket *ticket);

// Set configuration options ahead of connecting
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_cipher_suites(/* in */ mitls_state *state, const char *cs);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_signature_algorithms(/* in */ mitls_state *state, const char *sa);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_named_groups(/* in */ mitls_state *state, const char *ng);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_alpn(/* in */ mitls_state *state, const mitls_alpn *alpn, size_t alpn_count);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_early_data(/* in */ mitls_state *state, uint32_t max_early_data);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_custom_extensions(/* in */ mitls_state *state, const mitls_extension *exts, size_t exts_count);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_ticket_callback(mitls_state *state, void *cb_state, pfn_FFI_ticket_cb ticket_cb);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_nego_callback(mitls_state *state, void *cb_state, pfn_FFI_nego_cb nego_cb);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_configure_cert_callbacks(mitls_state *state, void *cb_state, mitls_cert_cb *cert_cb);

// Close a miTLS session - either after configure or connect
extern void MITLS_CALLCONV MiTLS_FFI_mitls_close(/* in */ mitls_state *state);
...
extern int MITLS_CALLCONV MiTLS_FFI_mitls_connect(void *send_recv_ctx, pfn_FFI_send psend, pfn_FFI_recv precv, /* in */ mitls_state *state);

// Act as a TLS server to a client
extern int MITLS_CALLCONV MiTLS_FFI_mitls_accept_connected(void *send_recv_ctx, pfn_FFI_send psend, pfn_FFI_recv precv, /* in */ mitls_state *state);

// Get the exporter secret (set early to true for the early exporter secret). Returns 1 if a secret was written
extern int MITLS_CALLCONV MiTLS_FFI_mitls_get_exporter(/* in */ mitls_state *state, int early, /* out */ mitls_secret *secret);

// Retrieve the server certificate after MiTLS_FFI_mitls_connect() completes
extern void *MITLS_CALLCONV MiTLS_FFI_mitls_get_cert(/* in */ mitls_state *state, /* out */ size_t *cert_size);

// Send a message
// Returns -1 for failure, or a TCP packet to be sent then freed with MiTLS_FFI_mitls_free()
extern int MITLS_CALLCONV MiTLS_FFI_mitls_send(/* in */ mitls_state *state, const unsigned char *buffer, size_t buffer_size);

// Receive a message
// Returns NULL for failure, a plaintext packet to be freed with MiTLS_FFI_mitls_free_packet()
extern unsigned char *MITLS_CALLCONV MiTLS_FFI_mitls_receive(/* in */ mitls_state *state, /* out */ size_t *packet_size);

// Free a packet returned MiTLS_FFI_mitls_*() family of APIs
extern void MITLS_CALLCONV MiTLS_FFI_mitls_free(/* in */ mitls_state *state, void* pv);
...
extern int MITLS_CALLCONV MiTLS_FFI_mitls_quic_create(quic_state **state, const quic_config *cfg);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_quic_process(quic_state *state, quic_process_ctx *ctx);

// get_record_secrets can be called after the complete flag is set
extern int MITLS_CALLCONV MiTLS_FFI_mitls_quic_get_record_key(quic_state *state, quic_raw_key *key, int32_t epoch, quic_direction rw);
extern int MITLS_CALLCONV MiTLS_FFI_mitls_quic_get_record_secrets(quic_state *state, quic_secret *crs, quic_secret *srs);
...
extern int MITLS_CALLCONV MiTLS_FFI_mitls_quic_send_ticket(quic_state *state, const unsigned char *ticket_data, size_t ticket_data_len);

// N.B. *cookie and *ticket_data must be freed with MiTLS_FFI_mitls_global_free as they are allocated in the global region
extern int MITLS_CALLCONV MiTLS_FFI_mitls_get_hello_summary(const unsigned char *buffer, size_t buffer_len, int has_record, mitls_hello_summary *summary, unsigned char **cookie, size_t *cookie_len, unsigned char **ticket_data, size_t *ticket_data_len);

// *ext_data points to a location in exts - no freeing required
extern int MITLS_CALLCONV MiTLS_FFI_mitls_find_custom_extension(int is_server, const unsigned char *exts, size_t exts_len, uint16_t ext_type, unsigned char **ext_data, size_t *ext_data_len);
...
// Free QUIC state
extern void MITLS_CALLCONV MiTLS_FFI_mitls_quic_free(quic_state *state);
