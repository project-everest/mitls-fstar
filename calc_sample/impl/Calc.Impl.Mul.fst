...
ensures exists* (resp_bytes1: bytes{Seq.length resp_bytes1 == 5}) (log1: calc_log).
  ...
  Vec.pts_to resp_buf resp_bytes1 **
  pure (
    log1 == step_log_mul req_bytes resp_bytes1 log0 /\
    ...
    serialize_response (snd (step log0.current_state Mul)) `Seq.equal` resp_bytes1
  )
{
  ...
  if SZ.gte csz 2sz {
    // Mul success: pop two, multiply, push result
    let val1 = Vec.op_Dot_Lparen_Rparen srv.stack (SZ.sub csz 2sz);  // second from top
    let val2 = Vec.op_Dot_Lparen_Rparen srv.stack (SZ.sub csz 1sz);  // top
    ...
    assert (pure (snd (step log0.current_state Mul) == Ok));
    assert (pure (serialize_response (snd (step log0.current_state Mul)) `Seq.equal` resp_bytes1));
    ...
    assert (pure (snd (step log0.current_state Mul) == Error));
    assert (pure (serialize_response (snd (step log0.current_state Mul)) `Seq.equal` resp_bytes1));
