...
(**
  ...
  The concrete stack logic (push when not full, else error) is unchanged from
  the hand-written codec, but the wire response is now produced by the shared
  [write_response] (the generated [response_writer]) and the ghost log is
  advanced by the single uniform [step_log].
**)
...
requires
  ...
  pure (
    ...
    (Ghost.reveal req).op == Push /\
    (Ghost.reveal req).operand == operand
  )
