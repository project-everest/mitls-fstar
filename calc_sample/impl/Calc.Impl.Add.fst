...
ensures exists* (resp_bytes1: bytes{Seq.length resp_bytes1 == 5}) (log1: calc_log).
  ...
  Vec.pts_to resp_buf resp_bytes1 **
  pure (
    log1 == step_log_add req_bytes resp_bytes1 log0 /\
    ...
    serialize_response (snd (step log0.current_state Add)) `Seq.equal` resp_bytes1
  )
{
  ...
  if SZ.gte csz 2sz {
    // Add success: pop two, add, push result
    ...
    assert (pure (snd (step log0.current_state Add) == Ok));
    assert (pure (serialize_response (snd (step log0.current_state Add)) `Seq.equal` resp_bytes1));
    ...
    assert (pure (snd (step log0.current_state Add) == Error));
    assert (pure (serialize_response (snd (step log0.current_state Add)) `Seq.equal` resp_bytes1));
