...
ensures exists* (resp_bytes1: bytes{Seq.length resp_bytes1 == 5}) (log1: calc_log).
  ...
  pure (
    ...
    serialize_response (snd (step log0.current_state Div)) `Seq.equal` resp_bytes1
  )
{
  ...
  if SZ.gte csz 2sz {
    let val1 = Vec.op_Array_Access srv.stack (SZ.sub csz 2sz);  // y (second from top)
    let val2 = Vec.op_Array_Access srv.stack (SZ.sub csz 1sz);  // x (top)
