...
(** Process a Div request: pop two operands, push (second / top), guarding
    against an underflow or a zero divisor (the top of stack). **)
  ...
  )
{
  ...
  if SZ.gte csz 2sz {
    let v_top = Vec.op_Array_Access srv.stack (SZ.sub csz 1sz);     // x (top, divisor)
    let v_second = Vec.op_Array_Access srv.stack (SZ.sub csz 2sz);  // y (second, dividend)
    ...
    if U32.eq v_top 0ul {
      ...
      // Div success: pop two, push (second / top)
      let result = U32.div v_second v_top;                          // div y x, as in step
