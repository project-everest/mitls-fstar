...
inline_for_extraction noextract let signatureScheme_repr_validator = LPPI.validate_u16

inline_for_extraction noextract let signatureScheme_repr_jumper = LPPI.jump_u16

inline_for_extraction noextract let signatureScheme_repr_reader = (PPB.leaf_reader_of_serialized (LPPI.read_u16' ()))
...
inline_for_extraction let read_maybe_signatureScheme_key : PPB.leaf_reader parse_maybe_signatureScheme_key =
    PPE.mk_read_maybe_enum_key signatureScheme_repr_reader signatureScheme_enum

let signatureScheme_reader =
 lemma_synth_signatureScheme_inj ();
 PPB.leaf_reader_of_reader (PPC.read_synth' (PPB.reader_of_leaf_reader read_maybe_signatureScheme_key) synth_signatureScheme synth_signatureScheme_inv)

inline_for_extraction let write_maybe_signatureScheme_key : LPS.l2r_leaf_writer (serialize_maybe_signatureScheme_key) =
  LPPS.l2r_leaf_write_maybe_enum_key signatureScheme_repr_writer signatureScheme_enum (_ by (LP.enum_repr_of_key_tac signatureScheme_enum))

let signatureScheme_writer =
  [@inline_let] let _ = lemma_synth_signatureScheme_inj (); lemma_synth_signatureScheme_inv () in
  LPC.l2r_leaf_write_synth write_maybe_signatureScheme_key synth_signatureScheme synth_signatureScheme_inv (fun x -> synth_signatureScheme_inv x)

inline_for_extraction let size_maybe_signatureScheme_key : LPS.leaf_size (serialize_maybe_signatureScheme_key) =
  LPS.leaf_size_constant_size serialize_maybe_signatureScheme_key 2sz (_ by (FStar.Tactics.norm [delta; iota; zeta; primops]; FStar.Tactics.smt ()))

let signatureScheme_leaf_size =
  [@inline_let] let _ = lemma_synth_signatureScheme_inj (); lemma_synth_signatureScheme_inv () in
  LPC.leaf_size_synth size_maybe_signatureScheme_key synth_signatureScheme synth_signatureScheme_inv (fun x -> synth_signatureScheme_inv x)

let read_signatureScheme = PPB.copyful_parse_leaf signatureScheme_reader

let free_signatureScheme = PPB.free_leaf

let write_signatureScheme = PPB.l2r_safe_writer_leaf signatureScheme_serializer 2sz signatureScheme_writer

let size_signatureScheme = PPB.l2r_safe_size_leaf signatureScheme_serializer 2sz
