...
//                );
    ...
    static member sendDHE (st:state, nsc:nextSecurityContext, ?sigAlgAndKey:(Sig.alg * Sig.skey), ?fp:fragmentationPolicy) : state * nextSecurityContext * FServerKeyExchange =
        ...
        in
        let kexdh = match nsc.secrets.kex with
            | DH(kexdh) -> kexdh
    ...
    /// <returns> Updated state * Updated next security context * FServerKeyExchange message record </returns>
    static member receiveECDHE (st:state, nsc:nextSecurityContext, ?check_sig:bool, ?minECDHsize:nat*nat): state * nextSecurityContext * FServerKeyExchange =
        let check_sig = defaultArg check_sig false in
        let st,fske =
    ...
    ///     Currently, the check is not implemented
    /// <returns> Updated state * FServerKeyExchange message record </returns>
    static member receiveECDHE (st:state, pv:ProtocolVersion, cs:cipherSuite, ?minECDHSize:nat) : state * FServerKeyExchange =
        LogManager.GetLogger("file").Info("# SERVER KEY EXCHANGE : FlexServerKeyExchange.receiveECDHE");
        let st,hstype,payload,to_log = FlexHandshake.receive(st) in
        ...
        | HT_server_key_exchange  ->
            ...
            | Correct (dhdb, p, e, alg, signature) ->
                ...
                in
                LogManager.GetLogger("file").Debug(sprintf "--- Curve : %A" curve);
                LogManager.GetLogger("file").Debug(sprintf "--- Compression : %A" comp);
                LogManager.GetLogger("file").Debug(sprintf "--- Public Point : %s ; %s" (Bytes.hexString(ecp_yX)) (Bytes.hexString(ecp_yY)));
                LogManager.GetLogger("file").Debug(sprintf "--- Signature algorithm : %A" (alg));
                LogManager.GetLogger("file").Debug(sprintf "--- Signature : %s" (Bytes.hexString(signature)));
                LogManager.GetLogger("file").Debug(sprintf "--- Payload : %s" (Bytes.hexString(payload)));
    ...
    /// <param name="sigKey"> Signature secret key associated to the algorithm </param>
    /// <returns> FServerKeyExchange message bytes * Updated state * FServerKeyExchange message record </returns>
    static member prepareECDHE (kexecdh:kexECDH, crand:bytes, srand:bytes, pv:ProtocolVersion, sigAlg:Sig.alg, sigKey:Sig.skey) : FServerKeyExchange * kexECDH =
    ...
    /// <param name="fp"> Optional fragmentation policy at the record level </param>
    /// <returns> Updated state * FServerKeyExchange message record </returns>
    static member sendECDHE (st:state, nsc:nextSecurityContext, ?sigAlgAndKey:(Sig.alg * Sig.skey), ?fp:fragmentationPolicy) : state * nextSecurityContext * FServerKeyExchange =
        let fp = defaultArg fp FlexConstants.defaultFragmentationPolicy in
        let sAlg,sKey =
            match sigAlgAndKey with
            | Some(sak) -> sak
            | None ->
                ...
                | Some(hint) ->
                    ...
                    | Some(_,sAlg,sKey) -> sAlg,sKey
        in
        let kexecdh = match nsc.secrets.kex with
    ...
    /// <param name="fp"> Optional fragmentation policy at the record level </param>
    /// <returns> Updated state * FServerKeyExchange message record </returns>
    static member sendECDHE (st:state, kexecdh:kexECDH, crand:bytes, srand:bytes, pv:ProtocolVersion, sigAlg:Sig.alg, sigKey:Sig.skey, ?fp:fragmentationPolicy) : state * FServerKeyExchange =
        LogManager.GetLogger("file").Info("# SERVER KEY EXCHANGE : FlexServerKeyExchange.sendECDHE");
        let fp = defaultArg fp FlexConstants.defaultFragmentationPolicy in
        ...
        LogManager.GetLogger("file").Debug(sprintf "--- Curve : %A" kexecdh.curve);
        LogManager.GetLogger("file").Debug(sprintf "--- Compression : %A" kexecdh.comp);
        LogManager.GetLogger("file").Debug(sprintf "--- Public Point : %s ; %s" (Bytes.hexString(ecp_xX)) (Bytes.hexString(ecp_xY)));
        LogManager.GetLogger("file").Debug(sprintf "--- Signature algorithm : %A" (fske.sigAlg));
        LogManager.GetLogger("file").Debug(sprintf "--- Signature : %s" (Bytes.hexString(fske.signature)));
        LogManager.GetLogger("file").Debug(sprintf "--- Payload : %s" (Bytes.hexString(fske.payload)));
        st,fske
