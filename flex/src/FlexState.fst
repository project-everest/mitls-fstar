...
let installReadKeys (st:state) (nsc:nextSecurityContext): state =
  Log.logDebug("@ Install Read Keys");
...
let installWriteKeys (st:state) (nsc:nextSecurityContext) : state =
  Log.logDebug("@ Install Write Keys");
...
let printBuffersStates (st:state) : unit =
  Log.logDebug("@ Buffers status (if not empty)");
  if not (st.read.hs_buffer = empty_bytes)   then Log.logDebug(sprintf "--- Handshake input buffer is not empty : %s" (Bytes.hexString(st.read.hs_buffer))) else
  if not (st.read.alert_buffer = empty_bytes) then Log.logDebug(sprintf "--- Alert input buffer is not empty : %s" (Bytes.hexString(st.read.alert_buffer))) else
  if not (st.read.appdata_buffer = empty_bytes) then Log.logDebug(sprintf "--- App Data input buffer is not empty : %s" (Bytes.hexString(st.read.appdata_buffer))) else
  if not (st.write.hs_buffer = empty_bytes)   then Log.logDebug(sprintf "--- Handshake output buffer is not empty : %s" (Bytes.hexString(st.write.hs_buffer))) else
  if not (st.write.alert_buffer = empty_bytes) then Log.logDebug(sprintf "--- Alert output buffer is not empty : %s" (Bytes.hexString(st.write.alert_buffer))) else
  if not (st.write.appdata_buffer = empty_bytes) then Log.logDebug(sprintf "--- App Data output buffer is not empty : %s" (Bytes.hexString(st.write.appdata_buffer))) else
  ()
