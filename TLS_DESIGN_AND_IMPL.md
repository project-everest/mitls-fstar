...
- `src/impl/TLS13.Connection.Core.fsti` / `.fst` now introduces the first buffer-oriented application-phase proof boundary. Its concrete buffer/request/response predicates and `process_request` theorem use explicit `network_in`, `app_in`, `network_out`, and `app_out` arrays and return a `ConnectionLog.step` witness. Handshake start is deliberately excluded. Application writes now install/use a record-state boundary and emit one sealed TLS application record into `network_out`, threading exactly that output prefix into the raw sent log before proving the application `ConnectionLog.step`. Reads still model an empty input poll, close still has a zero-network skeleton, and multi-record fragmentation remains future work.
...
- `TLS13.Connection.Core` now provides a `process_request`-style core theorem for application-phase buffers. The send path is connected to record/framing code for a single application record; close serialization, non-empty `network_in` reads, and multi-record fragmentation are still pending.
