...
- `TLS13.Connection.Core` now provides the first `process_request`-style core theorem for application-phase buffers, but its implementation still needs to be connected to the real record/framing code so `network_out` and non-empty `network_in` are meaningful TLS wire bytes.
