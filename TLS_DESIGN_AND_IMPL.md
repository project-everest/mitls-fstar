...
- `TLS13.Connection.Core` now provides a `process_request`-style core theorem for application-phase buffers. The send and close paths are connected to record/framing code for a single TLS record, and reads now consume explicit `network_in` into the raw received log. Decrypting received records into `app_out` and multi-record fragmentation are still pending.
