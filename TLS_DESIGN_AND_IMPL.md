...
- The strongest current proof surface is per-step preservation of
  ...
  epoch/sequence projection, and app-log projection from `cs_event_log`; the
  client theorem surface exposes preservation lemmas for
...
2. `ConnectionLog` and `Spec.ConnectionState` still need one stronger layered
   ...
   epoch/sequence projection, and app-log projection are now packaged into an
   explicit preserved spec invariant, but key-schedule, pending-buffer,
   decryption, and remaining event-projection facts still need to be connected
   into it.
...
4. Strengthen `TLS13.ConnectionLog` / `TLS13.Spec.ConnectionState` so raw bytes,
   ...
   epoch/sequence projection, and app-log-consistency preservation lemmas, with
