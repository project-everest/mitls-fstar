...
- `TLS13.Connection.Core` now provides a `process_request`-style core theorem for application-phase buffers. The send and close paths are connected to record/framing code for a single TLS record. Reads consume explicit `network_in` into the raw received log, decrypt one complete peer application record into `app_out`, map close_notify to `Closed`, and map known TLS alerts to `Failed (AlertError ...)`. Multi-record fragmentation and splitting one decrypted record across multiple reads are still pending.
