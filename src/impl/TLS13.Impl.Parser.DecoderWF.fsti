...
let l_is_received_cleartext (l:L.tls_message) : bool =
  match l with
  | L.LTlsHandshake (L.LClientHello _) -> true  // Servers receive ClientHello cleartext
...
val lemma_content_type_matches_injective (wire:U8.t) (c1 c2:T.content_type)
  : Lemma
...
(** The local representation and spec representation agree on what constitutes
    "cleartext when received". *)
val lemma_l_received_cleartext_matches
...
val lemma_cleartext_consistent_implies
...
val lemma_cleartext_tls_message_raw_of_parse
  (content_type:U8.t) (outer_ct:T.content_type)
...
val lemma_mk_cleartext_decoder_fragment_relation
...
val lemma_mk_cleartext_network_input_wf
  (st0:CS.connection_state) (content_type:U8.t) (outer_ct:T.content_type)
  (fragment:B.bytes) (raw:B.bytes)
  (l:L.tls_message) (m:M.tls_message)
  : Lemma
...
val lemma_mk_cleartext_network_input_wf_consistent
  (st0:CS.connection_state) (content_type:U8.t) (outer_ct:T.content_type)
  (fragment:B.bytes) (raw:B.bytes)
  (l:L.tls_message) (m:M.tls_message)
  : Lemma
...
val lemma_mk_cleartext_network_input_wf_none
  (st0:CS.connection_state) (content_type:U8.t) (outer_ct:T.content_type)
  (fragment:B.bytes) (raw:B.bytes)
  : Lemma
    (requires
...
val lemma_mk_protected_decoder_fragment_relation
  ...
  : Lemma
    (requires
      ...
      FStar.Seq.equal fragment plaintext.M.fragment)
...
val lemma_mk_protected_network_input_wf
  (st0:CS.connection_state) (content_type:U8.t) (fragment:B.bytes) (raw:B.bytes)
...
val lemma_mk_protected_network_input_wf_none
  (st0:CS.connection_state) (content_type:U8.t) (fragment:B.bytes) (raw:B.bytes)
  : Lemma
    (requires
      CT.protected_decoder_fragment_relation st0 content_type fragment raw /\
...
val lemma_parse_record_buffer_prefix
  ...
  : Lemma
    (requires
      ...
      prefix == FStar.Seq.slice raw 0 (5 + flen) /\
      (let b0 = U8.v (FStar.Seq.index raw 0) in
       b0 = 0x14 \/ b0 = 0x15 \/ b0 = 0x16 \/ b0 = 0x17) /\
      U8.v (FStar.Seq.index raw 1) = 0x03 /\
      U8.v (FStar.Seq.index raw 2) = 0x03 /\
      U8.v (FStar.Seq.index raw 3) * 256 + U8.v (FStar.Seq.index raw 4) == flen)
    (ensures
      (match WS.parse_record prefix with
       | None -> False
       | Some (ct, frag, consumed) ->
         consumed == 5 + flen /\
         FStar.Seq.equal frag (FStar.Seq.slice prefix 5 (5 + flen)) /\
         (U8.v (FStar.Seq.index raw 0) = 0x14 ==> ct == T.ChangeCipherSpec) /\
         (U8.v (FStar.Seq.index raw 0) = 0x15 ==> ct == T.Alert) /\
         (U8.v (FStar.Seq.index raw 0) = 0x16 ==> ct == T.Handshake) /\
         (U8.v (FStar.Seq.index raw 0) = 0x17 ==> ct == T.ApplicationData)))
