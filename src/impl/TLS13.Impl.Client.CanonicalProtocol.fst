...
let client_local_frame_post
  ...
  | CTypes.ClientGhostStep ->
    pure (
      result == CTypes.client_local_process_result {
...
let client_ghost_transition
  ...
  =
  {
    SM.tr_event = SM.LocalEvent CTypes.ClientGhostStep;
    SM.tr_next_state = st1;
    SM.tr_output = CPI.step_output [] [];
  }

let lemma_client_connection_step_state_ahead
  (initial:CS.connection_state)
  (st0:CS.connection_state)
  (st1:CS.connection_state)
  : Lemma
...
=
  ...
  let tr = client_ghost_transition st1 in
  assert (client_step
    st0
    (SM.LocalEvent CTypes.ClientGhostStep)
    st1
    tr.SM.tr_output);
...
let lemma_client_connection_state_evolves_state_ahead
  (initial:CS.connection_state)
  (st0:CS.connection_state)
  (st1:CS.connection_state)
  : Lemma
  ...
  RTC.induct
    ...
    (fun x ->
      SM.lemma_state_evolves_refl
        ...
        x)
    (fun x y ->
      lemma_client_connection_step_state_ahead initial x y)
    (fun x y z ->
      ...
      (Ghost.reveal current_state))
{
  ...
  MR.recall_snapshot
    (C.client_state_ref cc.canonical_client_state);
  lemma_client_connection_state_evolves_state_ahead
    (Ghost.reveal cc.canonical_client_initial)
    (Ghost.reveal snapshot_state)
    (Ghost.reveal current_state);
      ...
      local_outputs)
{
  ...
  match ev {
    ...
    CTypes.ClientGhostStep -> {
      assert (pure False);
    let result = CTypes.client_local_process_result {
      CT.network_out_len = 0sz;
      CT.app_out_len = 0sz;
      CT.status = CT.NeedMoreInput;
    };
