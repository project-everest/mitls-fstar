...
{
  let within_bound =
    SZ.lte certificate_chain_len
      Bounds.max_server_certificate_chain_len_sz;
  if within_bound {
    let credentials_opt =
           ...
           d st1 'certificate_chain 'credential_identity)
{
  rewrite
    ...
    (DS.top_server_driver_live
      d 'st0 'certificate_chain 'credential_identity);
  fold (BT.owns_server_transport_source None 'bind_host_bytes port);
  let result =
    BA.run d None bind_host bind_host_len port local_fuel network_fuel;
  unfold (BT.owns_server_transport_source None 'bind_host_bytes port);
  match result {
...
fn abort
  ...
  ensures
    exists* st certificate_chain credential_identity.
      DS.top_server_driver_closed d st certificate_chain credential_identity
{
  BC.abort d wire_received wire_sent pending app_log
}
