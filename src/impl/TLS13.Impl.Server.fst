...
module SApp = TLS13.Impl.Server.App
...
{
  let s =
    CR.new_server_erased_credential_identity
      certificate_chain
      certificate_chain_len
      #credential_identity;
  ST.lemma_initial_server_state_correct
...
{
  rewrite (connection_exactly s 'st0) as (SSch.connection_exactly s 'st0);
  let action = SSch.next_local_action s;
  rewrite (SSch.connection_exactly s 'st0) as (connection_exactly s 'st0);
  action
}
...
{
  rewrite (connection_exactly s 'st0) as (SSetup.connection_exactly s 'st0);
  let resp = SSetup.process_select_default_server_parameters_with_private_from_arrays
    s
    server_random
    server_private_key
    server_key_share
...
fn process_send_server_hello_from_arrays
  ...
  requires connection_exactly s 'st0 **
           ...
           pure (B.length 'server_random_bytes == 32 /\
                 ...
                 (Seq.length (Ghost.reveal 'server_random_bytes) == 32 ==>
                  (Ghost.reveal 'server_random_bytes <: Seq.lseq U8.t 32) <> GSHbody.serverHello_body_cst) /\
                 (let sh = SS.mk_server_hello_witness GNG.X25519 (Ghost.reveal 'server_random_bytes) (CryptoSpec.unpad_share_65 (Ghost.reveal 'server_key_share_bytes) 32) (CM.stored_client_hello_session_id 'st0) (CM.server_selected_suite 'st0) in
                 CM.can_send_server_hello
                   'st0
                   sh
                   (CS.serialized_cleartext_tls_message
...
{
  rewrite (connection_exactly s 'st0) as (SS.connection_exactly s 'st0);
  let resp = SS.process_send_server_hello_with_derived_public_from_private_array
    s
    server_random
    server_private_key
    network_out
                  ...
                  app_out_bytes)
{
  ...
  let resp = SApp.process_send_application_data_local_event
    s
    payload
    payload_len
    network_out
                  ...
                  app_out_bytes)
{
  assert (pure (SApp.server_app_local_event_input_ready
    ...
    (Ghost.reveal 'payload_bytes)));
  rewrite (connection_exactly s 'st0) as (SApp.connection_exactly s 'st0);
  let resp = SApp.process_send_close_notify_local_event
    s
    payload
    payload_len
    network_out
  ...
  match kind {
    ...
    ST.LocalSelectServerParameters -> {
      ...
      SMat.copy_server_random_and_private_from_payload
        ...
        server_random
        server_private_key;
      with server_random_bytes server_private_key_bytes.
      ...
      with st1 network_out_bytes app_out_bytes.
        assert (connection_exactly s st1 **
                ...
                pts_to network_out network_out_bytes **
                pts_to app_out app_out_bytes);
      ST.lemma_local_select_server_parameters_payload_irrelevant
        'st0
        st1
        resp
        B.empty
      ...
      if (network_out_len = 95sz `SZ.add` dispatch_sid_len) {
        ...
        with server_random_bytes server_private_key_bytes.
          assert (pts_to payload 'payload_bytes **
                  ...
                  pts_to app_out 'old_app_out);
        let resp =
          process_send_server_hello_with_derived_public_from_private_array
            s
            server_random
            server_private_key
            network_out
    ...
    ST.LocalSendCertificateVerify -> {
      ...
      assert (pure (Some?
        'st0.CS.cs_model.CS.model_handshake.CS.hs_certificate_verify));
      let cv : erased GCV.certificateVerify =
        Ghost.hide (Some?.v
          'st0.CS.cs_model.CS.model_handshake.CS.hs_certificate_verify);
      assert (pure (
        'st0.CS.cs_model.CS.model_handshake.CS.hs_certificate_verify ==
