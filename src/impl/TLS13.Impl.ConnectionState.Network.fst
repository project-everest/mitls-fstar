...
fn apply_received_server_finished
  ...
  ensures MR.pts_to c.ghost_state #1.0R st0 **
          ...
          ArrPts.pts_to raw 'raw_bytes
{
  let aead_key_len = read_negotiated_aead_key_len c;
  assert (pure (st0.CS.cs_model.CS.model_control ==
    CS.ControlHandshaking CS.HsCertificateVerifyVerified));
  ...
  KS.derive_traffic_key traffic_secret_out aead_key_len traffic_key_out;
  ...
  let material = Ghost.hide (CS.traffic_key_material_for_secret (TLS13.Crypto.Spec.aead_alg_of_key_len (SZ.v aead_key_len)) traffic_secret_bytes);
  ...
  Rec.install_application_keys_runtime c.records.read traffic_key_out aead_key_len (Ghost.hide (Ghost.reveal material).CS.traffic_key) traffic_iv_out;
          ...
          Pulse.Lib.Array.PtsTo.pts_to raw 'raw_bytes
{
  ...
  Rec.install_application_keys_runtime c.records.read traffic_key_out rotate_key_len (Ghost.hide (Ghost.reveal material).CS.traffic_key) traffic_iv_out;
