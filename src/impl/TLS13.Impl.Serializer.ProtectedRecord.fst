                     ...
                     Some (outer_fragment, R.next_seq (Ghost.reveal record_write)))))
{
  ...
  if sealed {
    ...
    assert (pure (CS.raw_records_exactly (Seq.slice network_bytes 0 (SZ.v written)) T.ApplicationData 1));
    assert (pure (Seq.equal
      (Seq.slice network_bytes 0 (SZ.v written))
      (WS.serialize_record T.ApplicationData ciphertext_bytes)));
    assert (pure (WS.parse_record (Seq.slice network_bytes 0 (SZ.v written)) ==
      Some (T.ApplicationData, ciphertext_bytes, SZ.v written)));
    assert (pure (Seq.equal
    ...
    assert (pure (R.seal
      (Ghost.reveal record_write)
      aad_bytes
      { R.content_type = T.ApplicationData; R.fragment = inner_plaintext_bytes } == None));
    Seq.lemma_eq_elim
      aad_bytes
      (CS.application_data_record_header (SZ.v handshake_len + 17));
    Seq.lemma_eq_elim
