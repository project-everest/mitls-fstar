...
noextract
let client_core_state_ref (c:client_core) : ST.state_ref = c.state

noextract
let client_core_log_ref (c:client_core) : ST.log_ref = c.log
...
fn finish_send_application_data_success
  (c: client_core)
  (app_in: array U8.t)
  (app_in_len: SZ.t)
  (network_out: array U8.t)
...
requires
  ST.current c.state view0.CL.state **
  ST.log_current c.log view0 **
  Rec.is_record_state c.client_application_record_state 'client_record_s1 **
  Rec.is_record_state c.server_application_record_state 'server_record_s0 **
  V.pts_to c.pending_read_buffer 'pending_read_buffer **
  Box.pts_to c.pending_read_offset 'pending_read_offset **
  Box.pts_to c.pending_read_len 'pending_read_len **
  V.pts_to c.pending_network_buffer 'pending_network_buffer **
  ...
  pure (
    ...
    SZ.v total_wire_len <= SZ.v network_out_cap /\
    view0.CL.state.S.phase == S.ApplicationData /\
    mreq == CL.request_no_network_in
      (CL.OpSendApplicationData (Ghost.reveal 'app_in_bytes)) /\
    U64.fits
      (view0.CL.state.S.write_state.R.seq +
       S.application_data_record_count (Ghost.reveal 'app_in_bytes)) /\
    'client_record_s1.R.seq ==
      view0.CL.state.S.write_state.R.seq +
      S.application_data_record_count (Ghost.reveal 'app_in_bytes) /\
    'server_record_s0.R.seq == view0.CL.state.S.read_state.R.seq /\
    CL.connection_view_consistent view0 /\
    CL.pending_app_source_consistent view0 /\
    V.is_full_vec c.pending_read_buffer /\
    V.length c.pending_read_buffer == SZ.v pending_read_buffer_capacity /\
    SZ.v 'pending_read_offset <= SZ.v 'pending_read_len /\
    SZ.v 'pending_read_len <= SZ.v pending_read_buffer_capacity /\
    Seq.equal view0.CL.pending_app
      (CL.raw_slice 'pending_read_buffer (SZ.v 'pending_read_offset) (SZ.v 'pending_read_len)) /\
    V.is_full_vec c.pending_network_buffer /\
    V.length c.pending_network_buffer == SZ.v pending_network_buffer_capacity /\
    SZ.v 'pending_network_len <= SZ.v pending_network_buffer_capacity /\
    Seq.equal view0.CL.pending_received_raw
      (CL.raw_slice 'pending_network_buffer 0 (SZ.v 'pending_network_len)))
...
fn finish_send_application_data_failure
  (c: client_core)
  (app_in: array U8.t)
  (app_in_len: SZ.t)
  (network_out: array U8.t)
...
fn process_send_application_data_request
  (c: client_core)
  (app_in: array U8.t)
  (app_in_len: SZ.t)
  (network_out: array U8.t)
      ...
      CL.step view0 mreq view1 mresp))
{
  ...
  if sealed.seal_records_ok {
    ...
    assert (pure (client_record_s1.R.seq ==
                  view0.CL.state.S.write_state.R.seq +
                  S.application_data_record_count (Ghost.reveal app_bytes)));
    finish_send_application_data_success
      c
      app_in
      app_in_len
      network_out
    ...
    finish_send_application_data_failure
      c
      app_in
      app_in_len
      network_out
...
requires
  ...
  pure (
    ...
    B.length 'app_out0 == SZ.v app_out_cap /\
    view0.CL.state.S.phase == S.ApplicationData /\
    U64.fits (view0.CL.state.S.write_state.R.seq + 1) /\
    mreq == CL.request_no_network_in CL.OpClose)
returns result: core_result
ensures exists* view1 network_out1 app_out1.
  is_client_core c view1 **
  pts_to network_out network_out1 **
...
fn process_pending_plaintext_read_request
  (c: client_core)
...
requires
  ST.current c.state view0.CL.state **
  ST.log_current c.log view0 **
  Rec.is_record_state c.client_application_record_state 'client_record_s0 **
  Rec.is_record_state c.server_application_record_state 'server_record_s0 **
  V.pts_to c.pending_read_buffer 'pending_read_buffer **
  Box.pts_to c.pending_read_offset 'pending_read_offset **
  Box.pts_to c.pending_read_len 'pending_read_len **
  ...
  pure (
    ...
    B.length 'app_out0 == SZ.v app_out_cap /\
    view0.CL.state.S.phase == S.ApplicationData /\
    Seq.equal (Ghost.reveal 'app_in_bytes) B.empty /\
    U64.fits
      (view0.CL.state.S.read_state.R.seq +
       B.length view0.CL.pending_received_raw +
       B.length (Ghost.reveal 'network_in_bytes) + 1) /\
    mreq == CL.request_with_network_in
      (CL.OpReadApplicationData (SZ.v requested_app_len))
    ...
    Seq.equal view0.CL.pending_received_raw
      (CL.raw_slice 'pending_network_buffer 0 (SZ.v 'pending_network_len)) /\
    record_states_match_view 'client_record_s0 'server_record_s0 view0)
returns result: core_result
ensures exists* view1 network_out1 app_out1.
  is_client_core c view1 **
  pts_to network_in 'network_in_bytes **
...
fn process_network_read_application_data_request
  (c: client_core)
  (network_in: array U8.t)
  (network_in_len: SZ.t)
  (app_in: array U8.t)
          ...
          if SZ.(5sz <=^ pending_network_len') {
            ...
            if header_parse_ok {
              ...
              if not (SZ.(fragment_len <=^ remaining_pending_len)) {
                ...
                V.to_vec_pts_to c.pending_network_buffer;
                let residual_cursor = 0sz;
                let residual_remaining_len = residual_len;
                assert (pure (SZ.v residual_cursor == 0));
                assert (pure (SZ.v residual_remaining_len == SZ.v residual_len));
                assert (pure (SZ.v residual_cursor + SZ.v residual_remaining_len == SZ.v residual_len));
                set_pending_network_from_slice
                  c
                  residual_tmp
                  residual_len
                  residual_cursor
                  residual_remaining_len;
                with pending_network_buffer2. assert (V.pts_to c.pending_network_buffer pending_network_buffer2);
                let pending_raw_payload : erased B.bytes =
                  CL.raw_slice
                    (Ghost.reveal residual_tmp_bytes)
                    (SZ.v residual_cursor)
                    (SZ.v residual_len);
                assert (pure (CL.raw_slice (Ghost.reveal residual_tmp_bytes) (SZ.v residual_cursor) (SZ.v residual_len) ==
                              Seq.slice (Ghost.reveal residual_tmp_bytes) (SZ.v residual_cursor) (SZ.v residual_len)));
                assert (pure (Seq.equal
                  (Ghost.reveal pending_raw_payload)
                  (CL.raw_slice (Ghost.reveal pending_network_buffer2) 0 (SZ.v residual_remaining_len))));
                ...
                if opened {
                  ...
                  if (inner_content_type = 23uy) {
                    if (SZ.(payload_len <=^ requested_app_len) && SZ.(payload_len <=^ app_out_cap)) {
                      ...
                      S.lemma_advance_read_records_one view0.CL.state;
                      ...
                      assert (pure (2 <=
                        B.length view0.CL.pending_received_raw +
                        B.length (Ghost.reveal network_bytes) + 1));
      ...
      if header_parse_ok {
        ...
        if (not (content_type = 23uy) || not (SZ.(16sz <^ fragment_len))) {
          ...
          if SZ.(pending_network_buffer_capacity <^ residual_len) {
            ...
            assert (pure (Seq.equal
              (Ghost.reveal pending_raw_payload)
              (CL.raw_slice (Ghost.reveal pending_network_buffer1) 0 (SZ.v residual_remaining_len))));
          let mut cipher = [| 0uy; fragment_len |];
...
fn process_read_application_data_request
  (c: client_core)
  (network_in: array U8.t)
  (network_in_len: SZ.t)
  (app_in: array U8.t)
      ...
      CL.step view0 mreq view1 mresp))
{
  ...
  if SZ.(pending_read_offset <^ pending_read_len) {
    process_pending_plaintext_read_request
      c
      network_in
      network_in_len
      app_in
    ...
    assert (pure (SZ.v pending_read_offset == SZ.v pending_read_len));
    process_network_read_application_data_request
      c
      network_in
      network_in_len
      app_in
      ...
      CL.step view0 mreq view1 mresp))
{
  if KSendApplicationData? kind {
    ...
    assert (pure (U64.fits
      (view0.CL.state.S.write_state.R.seq +
       S.application_data_record_count (Ghost.reveal app_bytes))));
    process_send_application_data_request
      c
      app_in
      app_in_len
      network_out
    ...
    let network_bytes : erased B.bytes = Ghost.reveal 'network_in_bytes;
    assert (pure (Seq.equal (Ghost.reveal 'app_in_bytes) B.empty));
    assert (pure (mreq == CL.request_with_network_in (CL.OpReadApplicationData (SZ.v requested_app_len)) (Ghost.reveal network_bytes)));
    assert (pure (U64.fits
      (view0.CL.state.S.read_state.R.seq +
       B.length view0.CL.pending_received_raw +
       B.length (Ghost.reveal network_bytes) + 1)));
    process_read_application_data_request
      c
      network_in
      network_in_len
      app_in
    ...
    assert (pure (mreq == CL.request_no_network_in CL.OpClose));
    assert (pure (U64.fits (view0.CL.state.S.write_state.R.seq + 1)));
