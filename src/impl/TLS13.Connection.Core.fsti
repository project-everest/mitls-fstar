...
(**
  ...
  16 KiB emit one record; writes over 16 KiB and up to 32 KiB emit two records.
  ...
  General write fragmentation beyond two records remains a later milestone.
**)
...
let request_record_sequence_fits
  ...
  match kind with
  | KSendApplicationData ->
    U64.fits (view.CL.state.S.write_state.R.seq + S.application_data_record_count app_in)
  | KClose ->
