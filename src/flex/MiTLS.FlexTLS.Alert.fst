...
let receive (st:state) : state * alertDescription * bytes =
  Log.write log Info "TLS Message" "# ALERT : FlexTLS.Alert.receive";
  ...
  if length buf < 2 then
    ...
    | Correct(ad) ->
      ...
      Log.write log Info "Field" (sprintf " --- Description : %A" ad);
...
let forward (stin:state) (stout:state) (*?*)(fp:fragmentationPolicy) : state * state * bytes =
  Log.write log Info "TLS Message" "# ALERT : FlexTLS.Alert.forward";
...
let send (st:state) (payload:bytes) (*?*)(fp:fragmentationPolicy) : state =
  Log.write log Info "TLS Message" "# ALERT : FlexTLS.Alert.send";
  // let fp = defaultArg fp FlexTLS.Constants.defaultFragmentationPolicy in
  ...
  let _ =
    if length payload = 2 then
      ...
      | Error(ad,x) -> failwith (perror __SOURCE_FILE__ __LINE__ x)
      | Correct(ad) -> Log.write log Info "Field" (sprintf "--- Description : %A" ad)
    else ()
  in
