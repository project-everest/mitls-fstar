...
let access_a : accessor parse_t parse_u16 (fun x y -> y == x.a) =
  accessor_weaken (
    accessor_synth
      (validate_nochk32_constant_size ((parse_u16 `nondep_then` parse_u32) `nondep_then` parse_u16) 8ul ())
      synth_t
      ()
    `accessor_compose`
    nondep_then_fst (validate_nochk32_constant_size (parse_u16 `nondep_then` parse_u32) 6ul ()) parse_u16
    `accessor_compose`
...
let access_c : accessor parse_t parse_u16 (fun x y -> y == x.c) =
  accessor_weaken (
    accessor_synth
      ...
      ()
    `accessor_compose`
    nondep_then_snd (validate_nochk32_constant_size (parse_u16 `nondep_then` parse_u32) 6ul ()) validate_nochk32_u16
  )
  (fun x y -> y == x.c) ()
