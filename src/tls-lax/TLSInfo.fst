...
val noMsId: i:msId { not (honestMS i) }                                                       let noMsId = StandardMS noPmsId noCsr PRF_SSL3_nested
...
let siId si r =
 ...
 let unAuthIdInv (i:id):epoch = //NS: TODO, please remove this use of the pre-processor
 (*#if verify
...
val macAlg_of_id: i:id { is_MtE i.aeAlg \/ (is_MACOnly i.aeAlg /\ is_SSL_3p0 (i.pv)) } -> Tot macAlg
...
val strongAuthId: i:id{((is_MtE i.aeAlg) \/ ((is_MACOnly i.aeAlg) /\ (is_SSL_3p0 i.pv)))} -> Tot bool
let strongAuthId i =
  match i.aeAlg with
...
assume val strongAEId: i:id -> Tot bool
(* this definition makes this function always false *)
(*let strongAEId i =
  is_MtE i.aeAlg &&
  strongAEAlg i.pv i.aeAlg*)

// ``We are idealizing integrity/confidentiality for this id''
let authId (i:id{((is_MtE i.aeAlg) \/ ((is_MACOnly i.aeAlg) /\ (is_SSL_3p0 i.pv)))}) = safeKDF i && strongAuthId i
...
val noId: i:id { ((is_MtE i.aeAlg) \/ ((is_MACOnly i.aeAlg) /\ (is_SSL_3p0 i.pv))) /\ (not (authId i)) }
let noId = {
  msId    = noMsId;
