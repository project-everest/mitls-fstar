...
let op_At_Bar (b1:B.bytes) (b2:B.bytes) = B.append b1 b2
...
val vlbytes_length_lemma: n:nat -> a:B.bytes{B.repr_bytes (B.length a) <= n} -> b:B.bytes{B.repr_bytes (B.length b) <= n} ->
  Lemma (requires ((B.slice (vlbytes n a) 0 n) = (B.slice (vlbytes n b) 0 n)))
...
let vlparse_vlbytes lSize vlb =
  ...
  let vl, b = B.split b' lSize in
  B.lemma_append_inj vl b (B.bytes_of_int lSize (B.length vlb)) vlb;
  assert (vl = (B.bytes_of_int lSize (B.length vlb)));
  B.int_of_bytes_of_int lSize (B.length vlb);
...
let namedGroupBytes ng =
  ...
  | SEC ec ->
    begin
    match ec with
    | ECC_P256		-> B.abyte2 (0x00z, 0x17z)
    | ECC_P384		-> B.abyte2 (0x00z, 0x18z)
    | ECC_P521		-> B.abyte2 (0x00z, 0x19z)
    | ECC_X25519  -> B.abyte2 (0x00z, 0x1dz)
    | ECC_X448    -> B.abyte2 (0x00z, 0x1ez)
    end
  | FFDHE dhe ->
    begin
    match dhe with
    | FFDHE2048		-> B.abyte2 (0x01z, 0x00z)
    | FFDHE3072		-> B.abyte2 (0x01z, 0x01z)
    | FFDHE4096		-> B.abyte2 (0x01z, 0x02z)
    | FFDHE6144		-> B.abyte2 (0x01z, 0x03z)
    | FFDHE8192		-> B.abyte2 (0x01z, 0x04z)
    end
  | NG_UNKNOWN u	-> B.abyte2 u

(* TODO: move to Platform.Bytes *)
let abyte2_inj x1 x2 : Lemma
  (B.abyte2 x1 == B.abyte2 x2 ==> x1 == x2)
  [SMTPat (B.abyte2 x1); SMTPat (B.abyte2 x2)]
= let s1 = B.abyte2 x1 in
  let s2 = B.abyte2 x2 in
  assert (x1 == (B.index s1 0, B.index s1 1));
  assert (x2 == (B.index s2 0, B.index s2 1))
...
private let lemma_namedGroupBytes_injective (ng1:namedGroup) (ng2:namedGroup)
  ...
  | SEC ec1, SEC ec2 ->
    if ec1 = ec2 then ()
    else assert(B.index nb1 0 = 0x00z /\ B.index nb2 0 = 0x00z)
  | FFDHE dhe1, FFDHE dhe2 ->
    if dhe1 = dhe2 then ()
    else assert(B.index nb1 0 = 0x01z /\ B.index nb2 0 = 0x01z)
  | NG_UNKNOWN u1, NG_UNKNOWN u2 ->
    abyte2_inj u1 u2
  | _ -> assert(B.index nb1 0 = B.index nb2 0 /\ B.index nb1 1 = B.index nb2 1)
#reset-options
...
let rec namedGroupsBytes0 groups =
  ...
  | [] -> B.empty_bytes
  | g::gs ->
    B.lemma_len_append (namedGroupBytes g) (namedGroupsBytes0 gs);
...
= match groups1, groups2 with
  ...
  | g1::groups1', g2::groups2' ->
    ...
    B.lemma_append_inj (namedGroupBytes g1) (namedGroupsBytes0 groups1') (namedGroupBytes g2) (namedGroupsBytes0 groups2');
