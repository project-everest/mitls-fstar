...
let protocol_versions_bytes = function
  ...
  | ClientPV vs -> vlbytes 1 (List.Tot.fold_left (fun acc v -> acc @| TLSConstants.versionBytes_draft v) empty_bytes vs)
...
let negotiateClientExtensions pv cfg cExtL sExtL cs ri resuming =
  ...
  | pv, Some cExtL, Some sExtL -> List.Tot.fold_left (serverToNegotiatedExtension cfg cExtL cs ri resuming) (correct pv) sExtL
