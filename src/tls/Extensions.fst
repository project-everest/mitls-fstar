...
*)
      | E_server_name _ ->
	  if List.Tot.existsb (fun (x: extension) ->match x with |E_server_name _ -> true | _ -> false) cExtL then
            correct(l)
	  else
      ...
      | E_pre_shared_key (ServerPSK n) ->
        Correct({l with ne_pskIndex = Some (UInt16.v n)})
