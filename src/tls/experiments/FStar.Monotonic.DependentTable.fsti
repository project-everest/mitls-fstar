module FStar.Monotonic.DependentTable
...
///    These maps provide three operations: empty, sel, upd
...
val empty (#a:eqtype) (#b:a -> Type) : r:map a b{repr r == empty_partial_dependent_map}
...
/// `grows r1 r2` is an abstract preorder on `imap` parametrized by a dependent preorder
/// `rel` on the values in the map.
/// The map can only be extended or updated respecting `rel`.
/// FStar.Monotonic.DependentMap is an instance of this with `rel k x y == (x == y)`.
///

(*
val grows (#a:eqtype) (#b:a -> Type) 
  (#inv:DM.t a (opt b) -> Type)
  (rel: (x:a) -> Preorder.preorder (b x)) 
  : Preorder.preorder (imap a b inv)
*)
let grows (#a:eqtype) (#b:a -> Type) 
  ...
  fun (m1:imap a b inv) (m2:imap a b inv) ->
    forall x.{:pattern (Some? (sel m1 x))}
           Some? (sel m1 x) ==>
              Some? (sel m2 x) /\
              rel x (Some?.v (sel m1 x)) (Some?.v (sel m2 x))


/// And, finally, the main type of this module:
///
/// `t r a b inv rel` is a mutable, imap stored in region `r` constrained
///               to evolve according to `grows rel`
let t (r:ST.erid) (a:eqtype) (b:a -> Type) 
  (inv:DM.t a (opt b) -> Type) 
  (rel: (x:a) -> Preorder.preorder (b x)) 
  = m_rref r (imap a b inv) (grows rel)
...
val defined_stable
    ...
    (t:t r a b inv rel)
    (x:a)
  : Lemma (stable_on (defined t x) t)

/// `contains_stable`: A property of the value of a key that is stable with respect to rel is stable with respect to `grows`
val contains_stable
    (#a:eqtype)
    (#b:a -> Type)
    (#inv:DM.t a (opt b) -> Type)
    ...
    (y:b x)
    (p:b x -> Type0)
    (p_stable_on_rel:squash (forall y z. p y /\ rel x y z ==> p z))  
  : Lemma (stable_on (fun h -> defined t x h /\ p (value_of t x h)) t)
...
val alloc (#a:eqtype) (#b:a -> Type) 
  (#inv:DM.t a (opt b) -> Type) 
  (#rel:(x:a) -> Preorder.preorder (b x)) 
  (#r:ST.erid)
  (_:unit{inv (repr empty)})
  : ST (t r a b inv rel)
       (requires fun h -> ST.witnessed (region_contains_pred r))
       (ensures  fun h0 x h1 -> ralloc_post r empty h0 x h1)

/// `update t x y`: Extending `t` with (x -> y)
///     Requires: - if `t` defines `x` then the update must respect `rel`
///               - the resulting heap would still respect `inv`
///     Ensures:  - that only `t` is modified
///               - by updating it to contain `(x -> y)`
///               - and in the future `t` will always be defined on `x`
val update
    (#a:eqtype)
    (#b:a -> Type)
    (#inv:DM.t a (opt b) -> Type)
  ...
  : ST unit
       (requires (fun h ->
         (defined t x h ==> rel x (value_of t x h) y) /\
       ...
       (ensures (fun h0 u h1 ->
         ...
         token_p t (defined t x)))

/// `lookup t x`: Querying the map `t` at point `x`
///      Ensures: - The state does not change
///               - If it returns `Some v`, then `t` will always be defined on `x`
val lookup
    (#a:eqtype)
    (#b:a -> Type)
    (#inv:DM.t a (opt b) -> Type)
...
let forall_t (#a:eqtype) (#b:a -> Type) 
  (#inv:DM.t a (opt b) -> Type) 
  (#rel:(x:a) -> Preorder.preorder (b x)) 
  (#r:ST.erid)
  (t:t r a b inv rel) 
...
val map_stable
    ...
    (#r:ST.erid)
    (t:t r a b inv rel)
    (p:(m:partial_dependent_map a b) -> Type0)
    (p_stable:squash (forall m1 m2. 
      p (repr m1) /\ grows #a #b #inv rel m1 m2 /\ inv (repr m1) ==> p (repr m2)))
  : Lemma (stable_on (fun h -> p (repr (HS.sel h t))) t)
