...
(* debug printing *)
let discard (_: bool) : ST unit
  (requires (fun _ -> True))
  (ensures (fun h0 _ h1 -> h0 == h1))
= ()
let print s = discard (IO.debug_print_string ("EPO| "^s^"\n"))
unfold let trace : (s:string) -> ST unit
  (requires (fun _ -> True))
  (ensures (fun h0 _ h1 -> h0 == h1))
...
type epoch_region_inv (#i:id) (hs_rgn:rgn) (r:reader (peerId i)) (w:writer i) =
  ...
  parent (region w) <> HS.root /\
  parent (region r) <> HS.root /\
...
val reveal_epoch_region_inv_all (u:unit)
  : Lemma (forall i hs_rgn r w.{:pattern (epoch_region_inv' #i hs_rgn r w)}
       epoch_region_inv' #i hs_rgn r w   <==>
       epoch_region_inv #i hs_rgn r w)

val reveal_epoch_region_inv (#hs_rgn:rgn) (#n:random) (e:epoch hs_rgn n)
...
val epochs_inv' (#r:rgn) (#n:random) (es: seq (epoch r n)) : GTot Type0

val reveal_epochs_inv' (u:unit)
...
let get_current_epoch
  ...
  (rw:rw)
  : ST (epoch r n)
       (requires (fun h -> 0 <= sel h (ctr e rw)))
       (ensures (fun h0 rd h1 ->
           let j = sel h1 (ctr e rw) in
