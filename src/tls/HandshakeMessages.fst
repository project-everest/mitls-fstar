...
val messageBytes_is_injective_strong:
  ...
  Lemma (requires True)
  (ensures (Bytes.equal (messageBytes ht1 data1 @| s1) (messageBytes ht2 data2 @| s2) ==> (ht1 = ht2 /\ Bytes.equal data1 data2 /\ Bytes.equal s1 s2)))
...
val messageBytes_is_injective:
  ...
  Lemma (requires True)
  (ensures (Bytes.equal (messageBytes ht1 data1) (messageBytes ht2 data2) ==> (ht1 = ht2 /\ Bytes.equal data1 data2)))
...
val clientHelloBytes_is_injective_strong: msg1:ch -> s1:bytes -> msg2:ch -> s2:bytes ->
  Lemma (requires True)
  (ensures (Bytes.equal (clientHelloBytes msg1 @| s1) (clientHelloBytes msg2 @| s2) ==> (msg1 == msg2 /\ s1 == s2)))
...
val serverHelloBytes_is_injective: msg1:valid_sh -> msg2:valid_sh ->
  Lemma (requires True)
  (ensures (Bytes.equal (serverHelloBytes msg1) (serverHelloBytes msg2) ==> msg1 == msg2))
      ...
      (match serverVer with
       | TLS_1p3 ->
          if length data >= 2 then
           let (csBytes, data) = split data 2ul in
...
val handshakeMessageBytes_is_injective: pv:option protocolVersion -> msg1:valid_hs_msg pv -> msg2:valid_hs_msg pv ->
  Lemma (requires True)
  (ensures (Bytes.equal (handshakeMessageBytes pv msg1) (handshakeMessageBytes pv msg2) ==> msg1 == msg2))
...
val handshakeMessagesBytes_is_injective: pv:option protocolVersion -> l1:list (msg:valid_hs_msg pv) -> l2:list (msg:valid_hs_msg pv) ->
  Lemma (requires True)
  (ensures (Bytes.equal (handshakeMessagesBytes pv l1) (handshakeMessagesBytes pv l2) ==> l1 = l2))
