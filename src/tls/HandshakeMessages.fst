...
let valid_crt: Type0 = c:crt{length (certificateListBytes c.crt_chain) < 16777212}

val certificateBytes: protocolVersion ->  valid_crt -> Tot (b:bytes{hs_msg_bytes HT_certificate b})
let certificateBytes pv crt =
  let cb = certificateListBytes crt.crt_chain in
...
let certificateBytes_is_injective pv c1 c2 =
  if certificateBytes pv c1 = certificateBytes pv c2 && pv = TLS_1p3 then (
    let cb1 = certificateListBytes c1.crt_chain in
    let cb2 = certificateListBytes c2.crt_chain in
    ...
    certificateListBytes_is_injective c1.crt_chain c2.crt_chain;
    ()
  ) 
...
let parseCertificate pv data = 
	...
	| Correct (certList) ->
		...
		if length certList < 16777212 then (
		  lemma_parseCertificateList_length certList;
		  Correct ({crt_chain = l})
		 )
		else Error(AD_decode_error, perror __SOURCE_FILE__ __LINE__ "")
            | Error z -> let (x,y) = z in Error(AD_bad_certificate_fatal, perror __SOURCE_FILE__ __LINE__ y))
...
let kex_c_of_dh_key kex = 
  ...
  | ECKey k -> KEX_C_ECDHE (serialize_ec_point k.ec_params k.ec_point)
...
assume val commonDH_serialize_is_injective: k1:CommonDH.key -> k2:CommonDH.key -> 
  Lemma (requires (True))
	(ensures (Seq.equal (serialize k1) (serialize k2) ==> k1 = k2))
...
let parseServerKeyExchange kex payload : result ske = 
    ...
    | Kex_DHE -> 
       (match parse_partial payload false with
	| Correct (k,sign) -> 
	  if length sign < 65536 then
        ...
        | Error z -> Error z)
    | Kex_ECDHE -> 
       (match parse_partial payload true with
	| Correct (k,sign) ->
	  if length sign < 65536 then
          Correct ({ske_kex_s = KEX_S_DHE k;
                    ske_sig = sign})
...
let valid_hs_msg: Type0 = msg:hs_msg{
  ...
  /\ (Certificate? msg ==> length (certificateListBytes (Certificate?._0 msg).crt_chain) < 16777212)}
