...
assume val to_bytes: l:nat -> buf:lbuffer l -> STL (b:bytes{length b = l})
...
#set-options "--z3rlimit 100 --initial_fuel 1 --max_fuel 1 --initial_ifuel 1 --max_ifuel 1"
assume val store_bytes: len:nat -> buf:lbuffer len -> i:nat{i <= len} -> b:bytes{length b = len} -> STL unit
  (requires (fun h0 -> Buffer.live h0 buf))
