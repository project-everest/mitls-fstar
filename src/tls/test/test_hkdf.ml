...
module Test
  ...
  let print_test_vector v =
    Printf.printf "IKM:\t%S\nsalt:\t%S\ninfo:\t%S\nL:\t%d\nPRK:\t%S\nOKM:\t%S\n"
      v.ikm v.salt v.info v.len v.prk v.okm

  let test v =
    let ikm  = Bytes.bytes_of_hex v.ikm in
    let salt = Bytes.bytes_of_hex v.salt in
    let info = Bytes.bytes_of_hex v.info in
    let prk = hkdf_extract v.hash salt ikm in
...
let main () = Test.(run_test "HKDF" test_vectors print_test_vector test)
