...
let sendHSRecord tcp pv log hs_msg = 
  let r = makePacket Content.Handshake pv hs_msg in
...
let sendAppDataRecord tcp pv msg = 
  let r = makePacket Content.Application_data pv msg in
  let Correct _ = Platform.Tcp.send tcp r in
  ()

let sendCCSRecord tcp pv = 
  let r = makePacket Content.Change_cipher_spec pv HandshakeMessages.ccsBytes in
  let Correct _ = Platform.Tcp.send tcp r in
  ()
...
*)
  ...
  let (Some k,ch,chb) = Handshake.prepareClientHello config None None in
