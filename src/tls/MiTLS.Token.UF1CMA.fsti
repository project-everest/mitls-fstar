...
val concrete_key_info (k:concrete_key) : info
val concrete_key_repr (k:concrete_key) : keyrepr (concrete_key_info k)
...
noeq (* abstract *) type ir_key (ip:ipkg) (i:ip.Pkg.t) =
  | IdealKey:
    ck: concrete_key ->
    region: Mem.subrgn (concrete_key_info ck).parent {~(is_tls_rgn region)} ->  // intuitively, the writer's region
    log: log_t i (concrete_key_info ck) region ->
    ir_key ip i
...
let usage (#ip:ipkg) (#i:ip.Pkg.t{ip.Pkg.registered i}) (k:key ip i): GTot info =
  if model then
    ...
    match k' with
    | IdealKey ck _ _ -> concrete_key_info ck
    | RealKey ck -> concrete_key_info ck
  else (concrete_key_info k)

let keyval (#ip:ipkg) (#i:ip.Pkg.t{ip.Pkg.registered i}) (k:key ip i): GTot (keyrepr (usage k)) =
  if model then
    let k' : ir_key ip i = k in
    match k' with
    | IdealKey ck _ _ -> concrete_key_repr ck
    | RealKey ck -> concrete_key_repr ck
  else concrete_key_repr k
