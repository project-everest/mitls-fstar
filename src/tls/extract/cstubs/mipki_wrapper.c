...
#include <mitlsffi.h>
#include <MiTLS_PKI.h>
#include <MiTLS_TLSConstants.h>
#include <MiTLS_Negotiation.h>
...
#define FStar_Pervasives_Native_option___uint64_t_MiTLS_Parsers_SignatureScheme_signatureScheme MiTLS_Negotiation_certNego
typedef MiTLS_TLSConstants_alpn_gc Prims_list__FStar_Bytes_bytes;
...
void
MiTLS_PKI_select_(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st, MiTLS_Parsers_ProtocolVersion_protocolVersion pv,
  FStar_Bytes_bytes sni, FStar_Bytes_bytes alpn, MiTLS_Parsers_SignatureSchemeList_signatureSchemeList sal,
  FStar_Pervasives_Native_option___uint64_t_MiTLS_Parsers_SignatureScheme_signatureScheme *res)
{
...
FStar_Pervasives_Native_option___uint64_t_MiTLS_Parsers_SignatureScheme_signatureScheme
MiTLS_PKI_select(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st, MiTLS_Parsers_ProtocolVersion_protocolVersion pv,
  FStar_Bytes_bytes sni, FStar_Bytes_bytes alpn,
  MiTLS_Parsers_SignatureSchemeList_signatureSchemeList sal)
{
  FStar_Pervasives_Native_option___uint64_t_MiTLS_Parsers_SignatureScheme_signatureScheme dst;
  MiTLS_PKI_select_(cbs, st, pv, sni, alpn, sal, &dst);
  return dst;
}
...
Prims_list__FStar_Bytes_bytes* MiTLS_PKI_format(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st, uint64_t cert)
{
  mipki_state *pki = (mipki_state*)cbs;
...
void MiTLS_PKI_sign_(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st,
  uint64_t cert, MiTLS_Parsers_SignatureScheme_signatureScheme *sa, FStar_Bytes_bytes tbs,
...
FStar_Pervasives_Native_option__FStar_Bytes_bytes MiTLS_PKI_sign(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st,
  uint64_t cert, MiTLS_Parsers_SignatureScheme_signatureScheme sa, FStar_Bytes_bytes tbs)
{
  FStar_Pervasives_Native_option__FStar_Bytes_bytes res;
  MiTLS_PKI_sign_(cbs, st, cert, &sa, tbs, &res);
  return res;
}

bool MiTLS_PKI_verify_(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st,
  Prims_list__FStar_Bytes_bytes *certs, MiTLS_Parsers_SignatureScheme_signatureScheme *sa,
...
bool MiTLS_PKI_verify(FStar_Dyn_dyn cbs, FStar_Dyn_dyn st,
  Prims_list__FStar_Bytes_bytes *certs, MiTLS_Parsers_SignatureScheme_signatureScheme sa,
  FStar_Bytes_bytes tbs, FStar_Bytes_bytes sig) {
  return MiTLS_PKI_verify_(cbs, st, certs, &sa, tbs, sig);
}
...
FStar_Dyn_dyn MiTLS_PKI_init(Prims_string cafile, Prims_list__Prims_string___Prims_string___bool *certs)
{
...
void MiTLS_PKI_tls_callbacks(FStar_Dyn_dyn x0, MiTLS_TLSConstants_cert_cb *dst)
{
  dst->app_context = x0;
  dst->cert_select_ptr = NULL;
  dst->cert_select_cb = MiTLS_PKI_select_;
  dst->cert_format_ptr = NULL;
  dst->cert_format_cb = MiTLS_PKI_format;
  dst->cert_sign_ptr = NULL;
  dst->cert_sign_cb = MiTLS_PKI_sign_;
  dst->cert_verify_ptr = NULL;
  dst->cert_verify_cb = MiTLS_PKI_verify_;
}
#else
MiTLS_TLSConstants_cert_cb MiTLS_PKI_tls_callbacks(FStar_Dyn_dyn x0)
{
  return (MiTLS_TLSConstants_cert_cb){
    .app_context = x0,
    .cert_select_ptr = NULL,
    .cert_select_cb = MiTLS_PKI_select,
    .cert_format_ptr = NULL,
    .cert_format_cb = MiTLS_PKI_format,
    .cert_sign_ptr = NULL,
    .cert_sign_cb = MiTLS_PKI_sign,
    .cert_verify_ptr = NULL,
    .cert_verify_cb = MiTLS_PKI_verify
  };
}
#endif

void MiTLS_PKI_free(FStar_Dyn_dyn pki)
{
