...
let genReader parent #i w =
  ...
  assert(HS.disjoint wr reader_r);
...
let decrypt #i d ad c =
  ...
  if authId i then
    ...
    let j : counter (alg i) = HST.op_Bang ctr in
    ...
    | Some text ->
      ...
      if StatefulPlain.parseAD i (LHAEPlain.parseAD ad) = Content.Change_cipher_spec && text <> Content.ccsBytes then
        None
      else if StatefulPlain.parseAD i (LHAEPlain.parseAD ad) = Content.Alert && (length text <> 2 || TLS.Result.Error? (Alert.parse text)) then
        None
      else
	begin
	ctr := j + 1;
