...
type ffdhe_params = { 
  dh_p       :bytes; 
  dh_g       :(bs:bytes{length bs = 1}); 
  dh_q       :option bytes; 
  safe_prime :bool; // cwinter: Does safe_prime == true mean it's checked and registered?
}

type explicit_params = dhp:CoreCrypto.dh_params{length dhp.CoreCrypto.dh_p < 65536 && length dhp.CoreCrypto.dh_g < 65536}

type ec_params = uncompressedPointRepresentation

type group =
  | Named of ng:namedGroup{is_ecffdhe ng}
  | Explicit of explicit_params

let make_ffdhe_params 
    (p:string{length (bytes_of_hex p) < 65536})
    (q:string)
  : Tot ffdhe_params
  = {
    dh_p = bytes_of_hex p;
...
// let params_of_group (g:group): Tot params =
