...
*)
module MiTLS.HandshakeMessages
open MiTLS
...
let cipherSuitesVLBytes (l: list cipherSuiteName { List.Tot.length l < 256 } ) : Tot (b: bytes { length b <= 65536 } ) =
...
let clientHelloBytes_is_injective_strong msg1 s1 msg2 s2 =
  if clientHelloBytes msg1 @| s1 = clientHelloBytes msg2 @| s2 then begin
    ...
    let csB1 : bytes = cipherSuitesVLBytes msg1.ch_cipher_suites in
    ...
    let csB2 : bytes = cipherSuitesVLBytes msg2.ch_cipher_suites in
...
//    lemma_vlbytes_inj_strong 2 csb1 (tail3_1 @| s1) csb2 (tail3_2 @| s2);
    let _ : squash (msg1.ch_cipher_suites == msg2.ch_cipher_suites) =
      assume (msg1.ch_cipher_suites == msg2.ch_cipher_suites) // cipherSuitesBytes_is_injective msg1.ch_cipher_suites msg2.ch_cipher_suites
    in
    //TODO bytes NS 09/27
...
let parseClientHello data =
  ...
  | Correct cv ->
    ...
    | Correct (x) ->
      ...
      match parseVLCipherSuites data with
      | None -> error "ciphersuites"
      | Some (clientCipherSuites, data) ->
...
let serverHelloBytes_is_injective msg1 msg2 =
  ...
  cut(length (cipherSuiteNameBytes msg1.sh_cipher_suite) = length (cipherSuiteNameBytes msg2.sh_cipher_suite));
  //TODO bytes NS 09/27
       ...
       | Correct (x) ->
         ...
         if length sid <= 32 then
           if length data >= 3 then
             ...
             (let cs = parseCipherSuiteName csBytes in
