...
val create: #rid:rid -> role -> ST (ks * random)
  ...
  (ensures fun h0 (r,_) h1 ->
    ...
    /\ modifies_rref rid (Set.singleton (Mem.as_addr state)) (HS.HS?.h h0) (HS.HS?.h h1))
...
val ks_client_init: ks:ks -> ogl: option (list valid_namedGroup)
  ...
  (ensures fun h0 ogxl h1 ->
    ...
    modifies (Set.singleton rid) h0 h1 /\
    modifies_rref rid (Set.singleton (Mem.as_addr st)) (HS.HS?.h h0) (HS.HS?.h h1))
...
let ks_client_13_ch ks (log:bytes) : ST (recordInstance)
  ...
  let rw = StAE.coerce Mem.root id (ckv @| civ) in
  let r = StAE.genReader Mem.root rw in
...
let ks_server_13_sh ks log =
  ...
  let w = StAE.coerce Mem.root id (skv @| siv) in
  let rw = StAE.coerce Mem.root id (ckv @| civ) in
  let r = StAE.genReader Mem.root rw in
...
let ks_12_record_key ks =
  ...
  let w = StAE.coerce Mem.root id (wk @| wiv) in
  let rw = StAE.coerce Mem.root id (rk @| riv) in
  let r = StAE.genReader Mem.root rw in
  StAEInstance r w
...
let ks_client_13_sh ks sr cs log (| g, gy|) accept_psk =
  ...
  let skv: StreamAE.key (peerId id) = sk in
  let siv: StreamAE.iv (peerId id)  = siv in
  let w = StAE.coerce Mem.root id (ckv @| civ) in
  let rw = StAE.coerce Mem.root id (skv @| siv) in
  let r = StAE.genReader Mem.root rw in
...
val ks_client_12_set_session_hash: ks:ks -> h:bytes -> ST (TLSPRF.key * recordInstance)
  ...
  (ensures fun h0 r h1 ->
    ...
    /\ modifies_rref (KS?.region ks) (Set.singleton (Mem.as_addr (KS?.state ks))) (HS.HS?.h h0) (HS.HS?.h h1))
