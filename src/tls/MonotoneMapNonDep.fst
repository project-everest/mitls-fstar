...
(* Partial, non-dependent maps *)
abstract type map' (a:Type) (b:Type) =
  f:(x:a -> Tot (option b))

(* let reveal (#a:Type) (#b:a -> Type) (m:map' a b) : GTot (x:a -> Tot (option (b x))) = m
let hide (#a:Type) (#b:a -> Type) (f : x:a -> Tot (option (b x))) : GTot (map' a b) = f *)

type map (a b:Type) (inv:map' a b -> Type0) =
  m:map' a b{inv m}

abstract let upd (#a:eqtype) #b (m:map' a b) (x:a) (y: b)
  : Tot (map' a b)
  = fun z -> if x = z then Some y else m z

abstract let sel #a #b (m:map' a b) (x:a)
  : Tot (option b)
  = m x
...
let map_contains #a #b (m1:map' a b) (m2:map' a b) (x:a) (y:b)
  : Lemma (requires (grows m1 m2))
    (ensures (Some? (m1 x) /\ m1 x == Some y ==> m2 x == Some y))
  = ()

let contains_stable #r #a #b #inv (m:t r a b inv) (x:a) (y:b)
  : Lemma (ensures (stable_on_t m (contains m x y)))
  = ()

let extend (#r:rid) (#a:eqtype) (#b:Type) (#inv:(map' a b -> Type0)) (m:t r a b inv) (x:a) (y:b)
  : ST unit
