...
let chain_up_aux (c:cert) : certes = admit (); {
  cert_data = c;
  extensions = [] //AR: 04/08: moving admit inside
}
#reset-options

let chain_up (l:chain): chain13= admit (); List.Tot.map #cert #certes chain_up_aux l
let chain_down (l:chain13): chain = admit (); List.Tot.map #certes #cert (fun x -> x.cert_data) l
