module MiTLS.EverCrypt
open MiTLS

module B = LowStar.Buffer

module Hacl = MiTLS.EverCrypt.Hacl
module OpenSSL = MiTLS.EverCrypt.OpenSSL
module Vale = MiTLS.EverCrypt.Vale
...
let vale_aes128_gcm_encrypt xkey (iv:uint8_p) (ad:uint8_p) (adlen:uint32_t)
  ...
  push_frame ();
  let open MiTLS.EverCrypt.Vale in
...
let aead_encrypt pkey iv ad adlen plaintext len cipher tag =
  ...
  if AEAD_AES128_GCM_VALE? k then
    ...
    Hacl.Chacha20Poly1305_32.encrypt cipher tag plaintext len ad adlen key iv
...
let aead_decrypt pkey iv ad adlen plaintext len cipher tag =
  ...
  if AEAD_AES128_GCM_VALE? k then
    ...
    let key = AEAD_CHACHA20_POLY1305_HACL?.k k in
    let r = Hacl.Chacha20Poly1305_32.decrypt plaintext cipher len ad adlen key iv tag in
    U32.(1ul -^ r)
  else if AEAD_OPENSSL? k then
    let key = AEAD_OPENSSL?.st k in
