...
module TInfo = MiTLS.TLSInfo
open FStar.UInt32

module HS = FStar.HyperStack
module U8 = FStar.UInt8
module U32 = FStar.UInt32
module E = MiTLS.EverCrypt
module LB = LowStar.Buffer
module CI = MiTLS.Crypto.Indexing
module Plain = MiTLS.Crypto.Plain
module CB = MiTLS.Crypto.Symmetric.Bytes
...
type id = i:TInfo.id{~(TInfo.PlaintextID? i) /\ AEAD? (TInfo.aeAlg_of_id i)}
let alg (i:id): aeadAlg = let AEAD aead _ = TInfo.aeAlg_of_id i in aead
...
let gen (i:id) (r:rgn) : ST (state i Writer)
  ...
  assume (MiTLS.EverCrypt.Specs.aead_create_pre h); //effectively False
...
let leak (#i:id) (#rw:rw) (st:state i rw)
  : ST (key i * salt i)
  (requires (fun h0 -> ~(TInfo.authId i)))
...
let adlen i = match TInfo.pv_of_id i with
  | TLS_1p3 -> 0 | _ -> 13
