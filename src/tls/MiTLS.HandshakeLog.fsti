...
open MiTLS.Mem
open MiTLS.HandshakeMessages // for pattern matching on messages
...
let eoflight =
  let open MiTLS.HandshakeMessages in function
  | ClientHello _
  | HelloRetryRequest _
  | EndOfEarlyData
  | ServerHello _
...
let narrowTag (a: Hashing.Spec.alg) (b:anyTag { len b = Hacl.Hash.Definitions.hash_len a}) : tag a = b
...
module Range = MiTLS.Range // for now
open MiTLS.Range
