...
val find_psk_key_exchange_modes: offer -> list Extensions.psk_kex // [] when not found; pick better representation?
val find_sessionTicket: offer -> option bytes // TODO refine
val find_clientPske: offer -> 
  option (
    identities: list Extensions.pskIdentity {let n = length (Extensions.pskiListBytes identities) in 6 < n /\ n < 65536} * 
...
noeq type mode =
  | Mode:
    ...
    n_server_extensions: option (se:list Extensions.extension{List.Tot.length se < 256}) ->
...
val find_early_data: offer -> option Extensions.earlyDataIndication 
...
val client_ClientHello: 
  ...
  oks: option CommonDH.clientKeyShare -> 
  St offer
  // [C_Init ==> C_Offer]
...
type extra_ext = list Extensions.extension // to be refined 
...
val server_ServerShare: 
  #region:rgn -> 
  t region Server ->
  oks:option CommonDH.serverKeyShare -> 
  app_exts: extra_ext ->
  St (result mode)
