...
val clientExtensionHeaderBytes: clientExtension -> Tot bytes
...
let parse_sni_list b : Result ((list serverName)) =
    let rec aux b =
...
//val clientExtensionPayloadBytes: clientExtension -> Tot bytes
...
//val clientExtensionBytes: clientExtension -> Tot bytes
...
assume val clientExtensionsBytes:
       cl:list clientExtension -> Tot (b:bytes{length b < ((List.length cl) * 65539)})
(*
let clientExtensionsBytes extL =
    let extBL = List.map (fun e -> clientExtensionBytes e) extL in
    let extB = List.fold_left (fun s l -> s @| l) empty_bytes extBL in
...
let addOnceClient ext extList =
    if List.existsb (sameClientExt ext) extList then
...
val parseClientExtensions: data:bytes -> known_cipher_suites -> Result (list clientExtension)
...
val prepareClientExtensions: config -> ConnectionInfo -> cVerifyData -> l:list clientExtension{List.length l < 256}
...
let serverToNegotiatedExtension cExtL (resuming:bool) cs res sExt : Result (negotiatedExtensions)=
    ...
    | Error(x,y) -> Error(x,y)
    | Correct(l) ->
        if List.existsb (sameServerClientExt sExt) cExtL then
            match sExt with
            | SE_renegotiation_info (cvd,svd) -> correct ({l with ne_renegotiation_info=Some(cvd,svd)})
            | SE_server_name ->
                if List.existsb (fun x->match x with CE_server_name _ -> true | _ -> false) cExtL then correct(l)
...
let serverExtensionHeaderBytes ext =
    ...
    | SE_server_name              -> abyte2 (0x00uy, 0x00uy)
...
assume val serverExtensionsBytes: sl:list serverExtension -> Tot (b:bytes{length b < ((List.length sl) * 65539)})
(*
...
let addOnceServer ext extList =
    if List.existsb (sameServerExt ext) extList then
        Error(AD_handshake_failure, perror __SOURCE_FILE__ __LINE__ "Same extension received more than once")
    else
        let res = ext::extList in
        correct(res)
...
let clientToServerExtension (cfg:config) (cs:cipherSuite) ((renegoCVD:cVerifyData),(renegoSVD:sVerifyData)) (resuming:bool) (cExt:clientExtension) : (option (serverExtension)) =
    match cExt with
...
let negotiateServerExtensions cExtL cfg cs (cvd,svd) resuming  :  (list serverExtension * negotiatedExtensions) =
    let server = List.choose (clientToServerExtension cfg cs (cvd,svd) resuming) cExtL in
    let negi = ne_default in
    let nego = List.fold_left (clientToNegotiatedExtension cfg cs (cvd,svd) resuming) negi cExtL in
    (server,nego)
