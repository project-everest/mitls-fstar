...
include MiTLS.Parsers.ProtocolVersion // for TLS_1p3, etc.
...
let parseVersion : pinverse_t versionBytes = fun x ->
...
val strongAuthAlg: protocolVersion -> aeAlg -> Tot bool
//  | MtE _ m | MACOnly m   -> int_cma (macAlg_of_id i)
//  | AEAD e m -> strongAEADalg

let strongAEAlg (_:protocolVersion) (_:aeAlg) : Tot bool= false
// let strongAEAlg pv ae = match ae with
//    | AEAD e m -> true
//    | MtE _ -> false

val strongAuthAE: pv:protocolVersion -> ae:aeAlg -> Lemma(strongAEAlg pv ae ==> strongAuthAlg pv ae)
...
(** AEAD salt sizes *)
let aeadSaltSize =
  let open MiTLS.EverCrypt in function // TLS 1.3 IV salt.
  | AES128_GCM       -> 4
  | AES256_GCM       -> 4
...
let aeadRecordIVSize =
  let open MiTLS.EverCrypt in function // TLS 1.2 explicit IVs
  | AES128_GCM       -> 8
  | AES256_GCM       -> 8
  | CHACHA20_POLY1305 -> 0
  | _                 -> 8 //recheck
...
include MiTLS.Parsers.SignatureScheme
...
let sigHashAlg_of_signatureScheme (s:supported_signatureScheme) : (sigAlg * hashAlg) =
  let open MiTLS.Hashing.Spec in
  match s with
...
let signatureScheme_of_sigHashAlg (sa:sigAlg) (ha:hashAlg) : signatureScheme =
  let open MiTLS.Hashing.Spec in
  match sa, ha with
...
include MiTLS.Parsers.SignatureSchemeList

let signatureSchemeListBytes (algs:signatureSchemeList)
: Tot (b:bytes{4 <= length b /\ length b < 65538}) =
  signatureSchemeList_serializer32 algs

let parseSignatureSchemeList: pinverse_t signatureSchemeListBytes = fun b ->
...
include MiTLS.Parsers.CompressionMethod // for NullCompression

inline_for_extraction
type compression = compressionMethod

(** Serializing function for compression algorithms *)
let compressionBytes: compression -> Tot (lbytes 1) =
...
let parseCompression: b:lbytes 1
...
let rec parseCompressions (b:bytes {length b <= 254})
...
let rec compressionMethodsBytes
...
// Only to be invoked with TLS 1.2 (hardcoded in previous versions)
let prfMacAlg_of_ciphersuite_aux: cipherSuite -> Tot (option macAlg) =
...
let sessionHashAlg: pv:protocolVersion -> cs:cipherSuite{pvcs pv cs} -> Tot hashAlg =
  fun pv cs ->
  let open MiTLS.Hashing.Spec in
  match pv with
...
let get_aeAlg: cs:cipherSuite{ CipherSuite? cs \/ CipherSuite13? cs } -> Tot aeAlg =
  fun cs ->
  match cs with
...
let encAlg_of_aeAlg: (pv:protocolVersion) -> (a:aeAlg { MtE? a }) -> Tot (encAlg * ivMode) =
  fun pv ae ->
  match pv,ae with
...
let macAlg_of_aeAlg: (pv:protocolVersion) -> (a:aeAlg { pv <> TLS_1p3 /\ ~(AEAD? a) }) -> Tot macAlg =
  fun pv ae ->
  match pv,ae with
...
let certTypeBytes: certType -> Tot (lbytes 1) =
  fun ct ->
  match ct with
  | RSA_sign     -> abyte 1z
  | DSA_sign     -> abyte 2z
...
let parseCertType: pinverse_t certTypeBytes =
  fun b ->
  match b.[0ul] with
  | 1z -> Correct RSA_sign
  | 2z -> Correct DSA_sign
...
let rec certificateTypeListBytes: ctl:list certType {List.length ctl <= 256} -> Tot (lbytes (List.Tot.length ctl)) =
  fun ctl ->
  match ctl with
  | [] -> empty_bytes
  | h::t ->
...
let rec parseCertificateTypeList: data:bytes -> Tot (list certType) (decreases (length data)) =
  fun data ->
...
let defaultCertTypes: bool -> cipherSuite -> HyperStack.All.ML (l:list certType{List.Tot.length l <= 1}) =
...
let rec distinguishedNameListBytes
...
let rec parseDistinguishedNameList
...
val mk_cert_cb
    ...
    (requires fun _ -> True)
    (ensures fun h0 _ h1 -> modifies_none h0 h1))) : cert_cb
...
val cert_select_cb (c:config) (pv:protocolVersion) (sni:bytes) (alpn:bytes) (sig:signatureSchemeList)
   : ST (option (cert_type * signatureScheme))
        (requires fun _ -> True)
        (ensures fun h0 _ h1 -> modifies_none h0 h1)

val cert_format_cb (c:config) (ct:cert_type)
   : ST (list cert_repr)
        (requires fun _ -> True)
        (ensures fun h0 _ h1 -> modifies_none h0 h1)

val cert_sign_cb (c:config) (ct:cert_type) (ss:signatureScheme) (tbs:bytes)
   : ST (option bytes)
        (requires fun _ -> True)
        (ensures fun h0 _ h1 -> modifies_none h0 h1)

val cert_verify_cb (c:config) (cl:list cert_repr) (ss:signatureScheme) (tbs:bytes) (sigv:bytes)
   : ST bool
        (requires fun _ -> True)
        (ensures fun h0 _ h1 -> modifies_none h0 h1)
