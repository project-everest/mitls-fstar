...
module SrvOut = TLS13.ConnectionState.ServerNoCcsOutputs
module CliIn = TLS13.ConnectionState.ClientNoCcsInputs
  ...
  (
    ...
    (
      ...
      PNTWL.lemma_wire_serialize_all_injective
        (SM.trace_wire_outputs server_trace)
        (WFSM.trace_input_messages client_trace);
      assert (SM.trace_wire_outputs server_trace ==
              WFSM.trace_input_messages client_trace);
      // STEP 5: the server never emits a CCS-content record.
      SrvOut.lemma_server_trace_outputs_ccs_free
        server_initial server_initial server server_trace;
      assert (forall (wm:CW.wire_message).
        L.memP wm (WFSM.trace_input_messages client_trace) ==>
        wm.CW.wm_content_type <> T.Change_cipher_spec);
      // client_initial starts with an empty event log.
      assert (client_initial.CS.cs_event_log == []);
      assert (CCShape.log_has_no_received_ccs client_initial.CS.cs_event_log);
      // STEP 6: a CCS-content-free input trace never logs a received CCS.
      CliIn.lemma_client_no_ccs_event_from_ccs_free_inputs
