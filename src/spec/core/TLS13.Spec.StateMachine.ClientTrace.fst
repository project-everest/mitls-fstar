module TLS13.Spec.StateMachine.ClientTrace

(**
  Lightweight client-only trace projection of the role-parametric TLS state
  machine.

  TLS13.Spec.StateMachine is authoritative. This compact projection is used only
  by log views and implementation refinement proofs that expose a client phase.
  Its explicit name prevents it from being mistaken for the protocol model.
**)
...
include TLS13.Spec.StateMachine.RecordFraming
