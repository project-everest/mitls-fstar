...
let lemma_send_client_hello_progress (ch:M.client_hello)
  : Lemma (match S.step S.initial (S.SendClientHello ch) with
           | Some s -> s.S.phase == S.ClientHelloSent /\ s.S.failure == None
           | None -> False)
  = ()

let lemma_server_hello_supported_progress (s:S.conn_state) (sh:M.server_hello)
  : Lemma (requires (s.S.phase == S.ClientHelloSent /\
                     H.is_supported_cipher_suite sh.M.cipher_suite == true))
...
let lemma_controlled_handshake_reaches_application_data
  ...
  (sf:M.finished)
  (cf:M.finished)
  : Lemma (requires H.is_supported_cipher_suite sh.M.cipher_suite == true)
