...
4. **Connection buffering and partial I/O**: the extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, application record key installation during connect, application write record header/inner-plaintext/seal construction, application read exact-byte loops over short transport reads, and application read record-header parse/open/TLSInnerPlaintext decode/payload copy. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus separate key export to connect-time key output, raw write of already-formed application records, and a single raw application-byte read primitive that may return short counts. Remaining work is to move arbitrary partial socket write buffering into Pulse, collapse C exact-read loops used by the handshake path, handle handshake records during application reads, close_notify, fragmentation/padding, and error propagation according to `TLS13.StateMachine`.
...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, connect-time application-key derivation into extracted buffers, raw application-record write, a single raw application-byte read primitive, and the remaining backend handle for the extracted connection.
   ...
   - Replaced in the wrapper path so far: extracted `TLS13.Connection` now owns application-data write chunking, write record header/inner-plaintext/seal construction, bounded read scheduling, exact-byte loops over short raw transport reads, read record-header parsing, application record open, TLSInnerPlaintext decode, and payload copy over extracted-owned application traffic keys and extracted application record-state objects, including installing application record keys during connect via extraction-safe runtime record helpers; the C backend no longer stores application traffic keys, no longer exposes a separate key-export ABI, no longer exposes bulk application read/write functions, no longer receives app key/IV arrays in per-record hooks, no longer decrypts or copies application-data records, and no longer performs exact-read loops for application-data records.
   - Remaining TLS-owned work to replace: encrypted-handshake buffering/copy helper shape, C exact-read loops still used by the handshake path, raw partial-write buffering, application-read handling of handshake records/alerts, close_notify, padding, and fragmentation handling.
