...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, connect-time application-key derivation into extracted buffers, single raw application-byte read/write primitives, and the remaining backend handle for the extracted connection.
