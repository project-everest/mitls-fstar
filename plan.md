...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read/decrypt handoff, connect-time application-key derivation into extracted buffers, per-record application-data seal/open hooks, and the remaining backend handle for the extracted connection.
   ...
   - Remaining TLS-owned work to replace: encrypted-handshake buffering/copy helper shape, server-handshake record-state ownership, per-record application seal/open hooks, raw partial-I/O buffering, close_notify and fragmentation handling.
