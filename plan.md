...
6. [in progress] Add Pulse `.fsti` contracts and C stubs for HACL*, OpenSSL, parser/serializer, randomness, clock, and I/O. Initial Pulse interfaces and placeholder C stub files are present and verify/compile-syntax; HACL* C snapshot wrappers for SHA-256, HMAC-SHA256, HKDF-SHA256/TLS HKDF labels, X25519, TLS record nonce construction, ChaCha20-Poly1305, contiguous TLS ciphertext+tag AEAD wrappers, and random bytes have C tests; OpenSSL X.509 validation has generated localhost certificates, hostname-validation tests, and RSA-PSS-RSAE-SHA256 signature verification tests over the validated leaf key; POSIX fd read/write and localhost TCP connect/close wrappers have tests. Remaining work is extracted-name wiring plus fuller wire/I/O implementations and broader vectors.
7. [in progress] Implement trusted C parser/serializer support for the scoped TLS messages and extensions, with `.fsti` contracts and vector tests. Initial TLS record-header, handshake-header, and TLSInnerPlaintext encode/decode support plus malformed-input tests are present.
8. [in progress] Implement and verify transcript hash and TLS 1.3 key schedule; validate with RFC 8448 vectors. The HACL wrapper tests now include RFC 8448 simple 1-RTT handshake-secret extraction and client handshake traffic-secret HKDF-Expand-Label vectors; verified Pulse key-schedule implementation remains pending.
9. [in progress] Implement and verify ChaCha20-Poly1305 record protection/unprotection. A Pulse-facing `TLS13.Record.fsti` contract now exposes record-state allocation/free, key installation, application-data seal, and application-data open operations tied to `TLS13.Record.Spec`; concrete implementation and RFC/vector coverage remain pending.
10. [in progress] Implement and verify the scoped client handshake. A Pulse-facing `TLS13.Handshake.fsti` now exposes the client handshake context and per-flight send/receive contracts, each tied to `TLS13.StateMachine.step`; concrete implementation, byte-level transcript/key work, and RFC/vector tests remain pending.
