...
1. **Transcript and handshake byte driver**: the first extracted `TLS13.Handshake.ByteDriver` slice now owns the scoped encrypted server-flight schedule: it reads until a complete pending handshake message exists using a fuelled recursive helper bounded by `max_encrypted_records_per_message`, performs exact-byte reads over raw short transport reads for encrypted-handshake records, parses encrypted-handshake TLS record headers in extracted `TLS13.Record.Framing`, handles ChangeCipherSpec compatibility records, enforces the exact EncryptedExtensions, Certificate, CertificateVerify, Finished order, and maps wrong type, incomplete flight, or lower-layer failure to handshake failure. `TLS13.Record` now owns server-handshake record state/open and client-Finished record seal sequence/nonce/AEAD orchestration in the wrapper path. `TLS13.Handshake.FlightState` now owns the ClientHello/ServerHello transcript buffers, encrypted server-handshake message buffer, server-handshake record state, server-flight cursor, parsed-message offset, CertificateVerify offset, Finished transcript checkpoint lengths, encrypted-handshake completion flags, Finished verify-data, and long-lived handshake secret/traffic-secret/key/IV material in extracted code. `TLS13.Handshake.Transcript` now routes both the ClientHello || ServerHello handshake-secret checkpoint and scoped ClientHello || ServerHello || server-flight transcript hash through extracted code, and performs the Finished verify-data equality check in extracted code. Remaining work is to move encrypted-handshake record open/decode/append and body parser implementations out of trusted C, and reduce initial ServerHello raw record I/O to the same minimal transport boundary.
...
1. `tls13_connection_probe.c`
   ...
   - Remaining TLS-owned work to replace: encrypted-handshake record open/decode/append helper shape, initial ServerHello exact-read path, application-read handling of handshake records/receive-side alerts, padding, and fragmentation handling.
