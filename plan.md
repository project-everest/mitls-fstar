...
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-connection-driver-krml` and `extract-handshake-driver-krml` now use exact `--extract_module` invocations and emit named KRML files for the verified driver control-flow spines using the verified project cache. `extract-connection-driver-c` and `extract-handshake-driver-c` translate those KRML files to C against explicit external C ABIs, and the driver binding tests link the generated drivers against mock implementations to check success and short-circuit failure paths. `extract-key-schedule-c` translates the verified `TLS13.KeySchedule` Pulse implementation to C against an explicit crypto external ABI, and `test-key-schedule-bindings` links that generated C to the HACL* snapshot wrappers and checks RFC 8448-derived key-schedule outputs. `extract-record-c` now translates the verified `TLS13.Record` Pulse implementation to C against the same trusted crypto ABI plus a small Pulse array-copy shim, and `test-record-bindings` checks extracted record seal/open behavior against HACL*. A temporary trusted `tls13_connection_probe` C backend now reuses the scoped HACL*/wire/OpenSSL probe logic behind that ABI so `test/test_extracted_connection_driver_openssl` can run the extracted verified connection driver and extracted verified handshake driver against the local OpenSSL TLS echo server. Replacing the trusted C backend with extracted verified byte-level connection code remains pending.
