...
2. `tls13_wire_stubs.c`
   - Still trusted for scoped ClientHello serialization, legacy diagnostic paths, and non-wrapper parser/serializer call sites.
   - Replaced in the wrapper path so far: record header write/read, TLSInnerPlaintext no-padding write/read including encrypted-handshake inner plaintext, scoped ServerHello parsing/key-share extraction, handshake header parse, CertificateVerify input construction, Certificate leaf-DER offset parsing for the scoped generated-certificate flight, and CertificateVerify body scheme/signature-length parsing.
