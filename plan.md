...
2. **Record framing and TLSInnerPlaintext**: route the scoped application-data and encrypted-handshake framing through verified extracted code. The extracted `TLS13.Record` wrapper owns AEAD seal/open state; `TLS13.Record.Framing` now provides extracted no-padding TLSInnerPlaintext encoding for the application-data write path, padded TLSInnerPlaintext content-type/payload-length decoding for the application-data read path, no-padding decoding for the encrypted-handshake read path, TLS 1.3 application-data record-header construction for writes, and record-header parsing for the read path. Advanced read/write buffering remains under the connection-I/O milestone.
...
4. **Connection buffering and partial I/O**: the extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, application record key installation during connect, application write record header/inner-plaintext/seal construction, application exact-byte loops over short transport reads/writes, application read record-header parse/open/TLSInnerPlaintext decode/payload copy, and send-side close_notify alert record construction/seal/write before TCP close. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus separate key export to connect-time key output plus single raw application-byte read/write primitives that may return short counts. Remaining work is to handle handshake records and close_notify alerts during application reads, fragmentation/padding, and error propagation according to `TLS13.StateMachine`.
...
The next highest-value implementation step is to add application-read stream buffering/fragmentation behavior, then audit the remaining `tls13_connection_probe.c` backend down to raw transport, HACL*/OpenSSL boundaries, and small extraction support shims.
