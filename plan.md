...
10. [in progress] Implement and verify the scoped client handshake. The pure handshake spec now models RFC 8446 server CertificateVerify input with the 64-space prefix, context string, separator, and transcript hash, and exposes Finished verification against the key-schedule verify-data function. A Pulse-facing `TLS13.Handshake.fsti` exposes the client handshake context and per-flight send/receive contracts, each tied to `TLS13.StateMachine.step`; concrete implementation and full byte-level handshake driver work remain pending.
