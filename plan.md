...
Create `TLS13.Parse.fsti` and `TLS13.Serialize.fsti` as Pulse/F* interfaces over extraction-ready verified parsers/serializers. Earlier trusted C wire stubs have been removed from the default build once the wrapper path moved to extracted framing/parsing:
...
1. **Transcript and handshake byte driver**: the extracted handshake path now owns ClientHello construction, ClientHello transcript storage, TCP connect sequencing, exact-byte writes over raw short transport calls, exact-byte reads over raw short transport calls, and TLS record-header parsing for both the initial ServerHello record and encrypted server-flight records, plus scoped supported-ServerHello body parsing/key-share extraction. `TLS13.Handshake` now invokes extracted `TLS13.Handshake.ByteDriver` directly for the encrypted server flight, so C no longer has a top-level `recv_encrypted_extensions` orchestration hook. `TLS13.Handshake.ByteDriver` owns the scoped encrypted server-flight schedule: it reads until a complete pending handshake message exists using a fuelled recursive helper bounded by `max_encrypted_records_per_message`, handles ChangeCipherSpec compatibility records, enforces the exact EncryptedExtensions, Certificate, CertificateVerify, Finished order, and maps wrong type, incomplete flight, or lower-layer failure to handshake failure. `TLS13.Record`/`TLS13.Handshake.FlightState` now own server-handshake record open, TLSInnerPlaintext decode, append into the extracted server-handshake buffer, pending-message completeness checks, pending-message type parsing, EncryptedExtensions acceptance, Certificate body parsing/leaf metadata extraction, CertificateVerify body parsing/signature metadata extraction, Finished body parsing, Finished verify_data storage, ServerHello transcript hashing, handshake traffic-key derivation, server Finished transcript/MAC verification, post-handshake application traffic-key derivation, client Finished verify_data/framing/record construction, and cursor advancement over the owned server-flight buffer. `TLS13.Handshake` now exact-writes the extracted-built ClientHello and generated client Finished record over raw write hooks, so the old C ClientHello/client-Finished construction paths are gone. `TLS13.Handshake.FlightState` now owns the ClientHello/ServerHello transcript buffers, encrypted server-handshake message buffer, server-handshake record state, server-flight cursor, parsed-message offset, CertificateVerify offset, Finished transcript checkpoint lengths, encrypted-handshake completion flags, Finished verify-data, and long-lived handshake secret/traffic-secret/key/IV material in extracted code. `TLS13.Handshake.Transcript` now routes both the ClientHello || ServerHello handshake-secret checkpoint and scoped ClientHello || ServerHello || server-flight transcript hash through extracted code, and performs the Finished verify_data equality check in extracted code. Remaining work is to handle application-read alerts/handshake records and reduce the residual C backend to raw transport plus crypto/X.509 boundaries.
2. **Record framing and TLSInnerPlaintext**: route the scoped application-data and encrypted-handshake framing through verified extracted code. The extracted `TLS13.Record` wrapper owns AEAD seal/open state; `TLS13.Record.Framing` now provides extracted no-padding TLSInnerPlaintext encoding for the application-data write path, padded TLSInnerPlaintext content-type/payload-length decoding for the application-data read path, no-padding decoding for the encrypted-handshake read path, TLS 1.3 application-data record-header construction for writes, and record-header parsing for the read path.
3. **Scoped parser/serializer replacement**: replace any residual parser/serializer scaffolding for the supported ClientHello, ServerHello, EncryptedExtensions, Certificate, CertificateVerify, Finished, TLSPlaintext, and TLSInnerPlaintext structures with verified extraction-ready Pulse/F* implementations tied to `TLS13.Wire.Spec`. The old trusted C wire stubs and standalone C parser/serializer tests have been removed from the build; remaining work is strengthening/exposing the extracted parser/serializer specs and coverage.
4. **Connection buffering and partial I/O**: the extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, application record key installation during connect, application write record header/inner-plaintext/seal construction, application exact-byte loops over short transport reads/writes, application read record-header parse/open/padded TLSInnerPlaintext decode/payload copy, extracted pending plaintext buffering for oversized application records, post-handshake handshake-record skipping, receive-side close_notify and peer-alert routing, and send-side close_notify alert record construction/seal/write before TCP close. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus combined connect/key export to a raw handshake-connect hook, a narrow extracted-flight-state application-key derivation hook, and single raw application-byte read/write primitives that may return short counts. Remaining work is broader fragmentation vectors and finer alert mapping according to `TLS13.StateMachine`.
...
11. Treat HACL*, OpenSSL, randomness, clock, trust store, and socket I/O as trusted boundaries with precise `.fsti` contracts. Parser/serializer logic should be extracted verified code, not a trusted C-stub boundary.
...
The default `make test-openssl-echo` client path now uses a concrete extracted connection wrapper type, extracted handshake wrapper state transitions, extracted handshake driver control flow, extracted encrypted-handshake byte-driver scheduling, extracted transcript hash routing and Finished comparison, extracted key schedule, extracted record seal/open, extracted TLSInnerPlaintext encode/decode, extracted record header construction/parsing, extracted ClientHello serialization, extracted ServerHello parsing/key-share extraction, extracted handshake header parsing, extracted Certificate parsing, and extracted CertificateVerify input/body parsing. The remaining trusted code in the default wrapper interop path is:

1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, narrow application-key derivation from extracted flight state, single raw application-byte read/write primitives, and the remaining backend handle for the extracted connection.
   ...
   - Replaced in the wrapper path so far: encrypted server-flight scheduling is invoked directly by extracted `TLS13.Handshake` through extracted `TLS13.Handshake.ByteDriver`, using a fuelled extracted recursive helper instead of a C hook or hard-coded nested cascade; encrypted-handshake exact-byte raw-read loops and TLS record-header parsing, server-handshake record state/open, encrypted-handshake TLSInnerPlaintext decode/append, pending-message completeness/type parsing, EncryptedExtensions acceptance, Certificate body parsing/leaf metadata extraction, CertificateVerify body parsing/signature metadata extraction, Finished body parsing/verify_data copy, client-Finished verify_data/framing/record construction, and client-Finished record state/seal sequence/nonce/AEAD orchestration, expected handshake message order, wrong-type/incomplete-flight failure routing, extracted `TLS13.Handshake` routing of certificate/CV/Finished completion checks, and extracted flight-state ownership of ClientHello/ServerHello transcript buffers, encrypted server-handshake message buffer, cursor/checkpoint/saw-flag plus Certificate leaf/signature metadata, CertificateVerify verification-success state, server Finished verify_data, and long-lived handshake secret/traffic-secret/key/IV bookkeeping. `TLS13.Handshake.ByteDriver.External.fsti` now tracks an abstract progress state and proves successful extracted scheduling reaches `Complete`.
   ...
   - Replaced in the wrapper path so far: extracted `TLS13.Connection` now owns application-data write chunking, write record header/inner-plaintext/seal construction, bounded read scheduling, exact-byte loops over short raw transport reads/writes, read record-header parsing, application record open, padded TLSInnerPlaintext decode, payload copy, extracted pending plaintext buffering for oversized application records, post-handshake handshake-record skipping, receive-side close_notify alert recognition/state advancement, receive-side peer-alert routing to an explicit alert failure, and send-side close_notify alert record construction/seal/write over extracted-owned application traffic keys and extracted application record-state objects, including installing application record keys during connect via extraction-safe runtime record helpers; the C backend no longer stores application traffic keys, no longer exposes a combined connect/key-export ABI, no longer exposes bulk application read/write functions, no longer receives app key/IV arrays in per-record hooks, no longer decrypts or copies application-data records, and no longer performs exact-read/exact-write loops for application-data records.
   - Remaining TLS-owned work to replace: broader fragmentation vectors and finer alert-description mapping.
2. `tls13_hacl_stubs.c` plus HACL* C snapshot
...
The next highest-value implementation step is to broaden fragmentation vectors and then audit the remaining `tls13_connection_probe.c` backend down to raw transport, HACL*/OpenSSL boundaries, and small extraction support shims.
...
10. [in progress] Implement and verify the scoped client handshake. The pure handshake spec now models RFC 8446 server CertificateVerify input with the 64-space prefix, context string, separator, and transcript hash, and exposes Finished verification against the key-schedule verify-data function. `TLS13.Handshake.fst` now implements the Pulse-facing handshake wrapper: it carries a monotonic `TLS13.State.state_ref`, calls a narrowed raw-operation ABI, and proves each per-flight operation advances the state token to the corresponding success phase or to `Failed` on error. The default wrapper interop now links layered extracted `TLS13.Handshake` plus `TLS13.Handshake.Driver`, so these state transitions are in the runtime path. The extracted handshake layer builds the scoped ClientHello, stores it into the extracted flight state for transcript use, connects the socket through a raw transport hook, exact-writes the ClientHello record, exact-reads the initial ServerHello record, parses the ServerHello/key share, invokes the extracted encrypted-flight byte driver directly, checks extracted flight-state completion flags for Certificate/CertificateVerify/Finished progress through narrow hooks, invokes extracted server Finished verification through a narrow hook, and exact-writes the generated client Finished record. The extracted byte-driver layer has a stronger local progress contract (`ExpectEncryptedExtensions` through `Complete`) for the encrypted server flight, and `TLS13.Handshake.FlightState` now owns the runtime server-flight cursor/checkpoint/saw-flag state plus Certificate leaf, CertificateVerify signature metadata, Finished data, and handshake/application traffic material. `TLS13.Handshake.External.fsti` now mostly represents raw connect/read/write, OpenSSL-backed certificate/signature validation, and narrow flight-state accessors while those are being collapsed. The pure state machine makes successful certificate-chain validation an explicit `ValidateCertificate` event that stores the validated peer before CertificateVerify can advance. A ghost-only `TLS13.Handshake.StateDriver` verifies the controlled handshake by advancing the monotonic state token through every expected handshake event to `ApplicationData`, and `TLS13.Handshake.Driver` verifies the non-ghost control-flow spine that sequences the trusted handshake operations to either `ApplicationData` or `Failed`. `extract-handshake-c`, `extract-handshake-driver-c`, `extract-handshake-layered-c`, and `extract-handshake-flight-state-c` now emit C for the verified wrapper/driver/flight-state pieces; binding/OpenSSL tests check success and failure paths. Remaining work is to push the OpenSSL certificate/signature validation boundary to a smaller ABI and leave only raw transport plus crypto/X.509 boundaries.
11. [in progress] Implement and verify top-level connection read/write/close APIs. `TLS13.Connection.fst` now implements the top-level Pulse connection wrapper with a concrete extracted runtime `connection` struct around a lower trusted backend handle: allocation creates the backend handle, a runtime lifecycle marker, extracted-owned client/server application traffic key/IV buffers, extracted-owned client/server application record-state objects, extracted pending-read plaintext buffer, and the monotonic state token; connect proves the controlled handshake transition to `ApplicationData` or `Failed`, derives application keys through a narrow extracted-flight-state hook after handshake completion, installs application keys directly into extracted buffers, and installs those keys into extracted application record states via runtime record helpers; write/read advance record sequence state on successful application-data operations; and close advances to `Closing` or `Failed`. `TLS13.Connection` now also owns the application-data multi-record write chunking loop, per-record write framing/seal construction, bounded read scheduling loop, exact-byte loops over short raw transport reads/writes, raw record-header parsing, application record open, padded TLSInnerPlaintext decode, payload copy, pending plaintext buffering for oversized application records, post-handshake handshake-record skipping, receive-side close_notify alert recognition/state advancement to `Closed`, receive-side peer-alert routing to `AlertError DecodeError`, and close_notify alert record construction/seal/write over those extracted-owned keys/record states, replacing the old trusted bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` and combined connect/key-export external call with separated handshake-connect, application-key-derivation, and single raw read/write primitives. `TLS13.Connection.Driver.connect_write_read_exact` verifies the end-to-end connect/write/read control-flow spine to either remain in `ApplicationData` on success, reach `Closed` on peer close_notify, or reach `Failed` on failure. Application-data and close_notify state-machine transitions advance the corresponding record-layer read/write sequence state, and a ghost-only `TLS13.Connection.StateDriver` verifies application-data send/receive echo transitions and close_notify progress over the same monotonic token. `extract-connection-c` and `extract-connection-driver-c` now emit C for the verified wrapper and driver; binding/OpenSSL tests check success and failure paths, including partial raw reads/writes, split reads from one decrypted record, encrypted close_notify, encrypted fatal alerts, and padded application records. Remaining connection work is broader fragmentation vectors and finer alert-description mapping.
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-key-schedule-c`, `extract-record-c`, `extract-record-framing-c`, `extract-handshake-framing-c`, `extract-handshake-flight-state-c`, `extract-handshake-transcript-c`, `extract-handshake-byte-driver-c`, `extract-handshake-c`, `extract-handshake-driver-c`, `extract-handshake-layered-c`, `extract-connection-c`, and `extract-connection-driver-c` now translate verified Pulse/F* modules to C against explicit trusted ABIs. Binding tests link the generated C against mock or HACL-backed implementations to check success/failure paths and key/record vectors. The OpenSSL echo smoke now has a single client route: `test/test_extracted_connection_wrapper_openssl` runs extracted `TLS13.Connection`, extracted `TLS13.Handshake`, extracted handshake driver, extracted encrypted-handshake byte driver, extracted handshake flight-state, extracted handshake transcript wrapper, extracted key schedule, extracted record wrapper, extracted no-padding record framing, extracted ClientHello/ServerHello/Certificate/CertificateVerify framing, and extracted CertificateVerify input framing against the scoped HACL*/OpenSSL/I/O probe backend, and checks successful multi-record echo with split application reads plus wrong-CA rejection. Replacing the remaining trusted X.509, crypto, raw I/O, and connection-buffering backend pieces remains pending.
...
19. [in progress] Replace trusted C parser/serializer stubs with verified extraction-ready Pulse/F* parsers/serializers for the scoped TLS 1.3 message set, tied to `TLS13.Wire.Spec`. Trusted C wire stubs are no longer built; remaining work is stronger extracted parser/serializer specs and broader vectors.
20. [in progress] Implement extracted connection I/O loops for partial reads/writes, multi-record app data, close_notify, and handshake records observed during application reads, leaving only raw socket read/write as trusted. Current slices: extracted `TLS13.Connection` owns application write chunking, application write record construction/seal, bounded read scheduling, application record-state ownership, application record-key installation, exact-byte loops over short raw reads/writes, read record-header parsing, application record open, padded TLSInnerPlaintext decode, payload copy, pending plaintext buffering for oversized application records, post-handshake handshake-record skipping, receive-side close_notify alert recognition/state advancement, receive-side peer-alert routing, and send-side close_notify alert record construction/seal/write; the backend now exposes only raw short-read/write primitives for application data plus raw fd close.
