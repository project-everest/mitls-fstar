...
The controlled OpenSSL echo test now has three client-side paths:

1. A direct C probe path, which is useful for debugging the scoped OpenSSL handshake and echo behavior but is not a verified client path.
2. The older extracted-driver path, which runs extracted `TLS13.Connection.Driver` and `TLS13.Handshake.Driver` over trusted byte operations.
3. The primary extracted-wrapper path, which runs extracted `TLS13.Connection`, extracted `TLS13.Handshake.Driver`, extracted `TLS13.KeySchedule`, extracted `TLS13.Record`, and extracted `TLS13.Record.Framing` in the interop test. This path proves the modeled connection/handshake sequencing, key-schedule label/derivation orchestration, Finished verify-data routing, application record seal/open wrapper behavior, and no-padding application record framing before calling the remaining trusted byte backend.
...
1. **Transcript and handshake byte driver**: replace the large trusted `tls13_connection_probe.c` handshake loop with extracted Pulse code that owns transcript byte accumulation, handshake record boundaries, encrypted-handshake buffering, Finished transcript checkpoints, and failure transitions. Keep only primitive I/O, HACL*, OpenSSL X.509/signature calls, and low-level parser/serializer calls behind trusted ABIs during this step.
2. **Record framing and TLSInnerPlaintext**: route all application record framing through verified extracted code, including record header AAD construction, inner plaintext content-type/padding encoding and decoding, sequence-number overflow checks, and buffering of partial records. The extracted `TLS13.Record` wrapper already owns AEAD seal/open state; `TLS13.Record.Framing` now provides extracted no-padding TLSInnerPlaintext encoding for the application-data write path, no-padding content-type/payload-length decoding for the application-data read path, and TLS 1.3 application-data record-header construction for writes. Remaining work is padding support and read/write buffering.
...
6. **Interop gate update**: the extracted-wrapper path is now the primary `make test-openssl-echo` client path. The direct C probe and older extracted-driver path remain diagnostic/regression checks. Remaining interop-gate work is adding fragmentation/partial-read and close_notify coverage to the wrapper path before considering public HTTPS.
...
8. [in progress] Implement and verify transcript hash and TLS 1.3 key schedule; validate with RFC 8448 vectors. The pure key-schedule spec now includes fixed TLS 1.3 labels, early/handshake/master secret derivation, handshake/application traffic secrets, Finished-key/verify-data computation, exporter/resumption master secrets, and traffic key/IV derivation; `TLS13.KeySchedule.fsti` exposes Pulse-facing contracts for the named derivation steps; HACL wrapper tests include RFC 8448 simple 1-RTT handshake-secret extraction, client handshake traffic-secret HKDF-Expand-Label, transcript-hash, Finished-key, and server Finished verify-data vectors. `TLS13.KeySchedule.fst` now provides a verified Pulse implementation of the key-schedule wrapper layer: it constructs the exact TLS 1.3 labels with byte literals, uses stack-local Pulse arrays for temporaries, sequences the trusted crypto primitives, and proves each exported function writes the corresponding pure `TLS13.Keys` result. The implementation still relies on the trusted `TLS13.Crypto` primitive boundary.
9. [in progress] Implement and verify ChaCha20-Poly1305 record protection/unprotection. `TLS13.Record.fst` now implements the Pulse-facing record-state wrapper with concrete vector-backed key/IV storage, boxed sequence/install state, verified key installation, application-data seal, and application-data open operations tied to `TLS13.Record.Spec`; the record epoch remains ghost/spec-only because it has no runtime role in AEAD protection. `TLS13.Record.Framing.fst` now verifies and extracts no-padding TLSInnerPlaintext encoding for application-data writes, no-padding content-type/payload-length decoding for application-data reads, and TLS 1.3 application-data record-header construction for writes; the extracted-wrapper OpenSSL interop path uses these around extracted `TLS13.Record` seal/open. `extract-record-c` translates the verified wrapper to C against the trusted crypto ABI, and `test-record-bindings` links it to HACL* to check uninstalled-state rejection, application-data seal/open, and tamper rejection. C smoke coverage also combines TLSInnerPlaintext, record AAD, nonce construction, ChaCha20-Poly1305 seal/open, and tamper rejection. Remaining record work includes padding support, read/write buffering, and broader RFC record vectors.
...
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-key-schedule-c`, `extract-record-c`, `extract-record-framing-c`, `extract-handshake-c`, `extract-handshake-driver-c`, `extract-connection-c`, and `extract-connection-driver-c` now translate verified Pulse/F* modules to C against explicit trusted ABIs. Binding tests link the generated C against mock or HACL-backed implementations to check success/failure paths and key/record vectors. The OpenSSL echo smoke now uses the extracted wrapper route as the primary client path: `test/test_extracted_connection_wrapper_openssl` runs extracted `TLS13.Connection`, extracted handshake driver, extracted key schedule, extracted record wrapper, and extracted no-padding record framing against the scoped HACL*/wire/OpenSSL probe backend, and checks both successful multi-record echo and wrong-CA rejection. The older extracted driver route and direct C probe remain as regression/diagnostic coverage. Replacing the remaining trusted byte parser, X.509, crypto, and I/O backend pieces remains pending.
...
17. [pending] Implement the extracted handshake byte driver so the OpenSSL echo path no longer relies on `tls13_connection_probe.c` for transcript accumulation, encrypted-handshake record scheduling, Finished transcript checkpoints, or handshake failure transitions.
