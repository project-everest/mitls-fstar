...
4. **Connection buffering and partial I/O**: the first extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, and application record key installation during connect. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus separate key export to connect-time key output and per-application-record hooks that no longer receive key/IV arrays or inspect record-state internals. Remaining work is to move each per-record seal/open and raw record read/write operation fully into Pulse, handle arbitrary partial socket reads/writes, handshake records during application reads, close_notify, and error propagation according to `TLS13.StateMachine`.
...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, connect-time application-key derivation into extracted buffers, per-record application-data seal/open hooks, and the remaining backend handle for the extracted connection.
