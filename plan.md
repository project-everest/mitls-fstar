...
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-connection-driver-krml` now emits KRML for the verified `TLS13.Connection.Driver` control-flow spine using the verified project cache. KaRaMeL C generation for that driver is intentionally still blocked until the trusted `TLS13.Connection` operations are given external C bindings, so full `TLS13.Connection` extraction and trusted-stub linking remain pending.
