...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read/append, transcript byte storage, OpenSSL peer handle storage, connect-time application-key derivation into extracted buffers, per-record application-data seal/open hooks, and the remaining backend handle for the extracted connection.
