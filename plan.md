...
8. [in progress] Implement and verify transcript hash and TLS 1.3 key schedule; validate with RFC 8448 vectors. The pure key-schedule spec now includes fixed TLS 1.3 labels, early/handshake/master secret derivation, handshake/application traffic secrets, exporter/resumption master secrets, and traffic key/IV derivation; HACL wrapper tests include RFC 8448 simple 1-RTT handshake-secret extraction and client handshake traffic-secret HKDF-Expand-Label vectors. Verified Pulse key-schedule implementation remains pending.
