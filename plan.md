...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read/append, remaining encrypted-handshake message byte storage, OpenSSL peer handle storage, connect-time application-key derivation into extracted buffers, per-record application-data seal/open hooks, and the remaining backend handle for the extracted connection.
   ...
   - Remaining TLS-owned work to replace: encrypted-handshake message byte-array ownership/copying, encrypted-handshake buffering, per-record application seal/open hooks, OpenSSL peer lifetime, raw partial-I/O buffering, close_notify and fragmentation handling.
