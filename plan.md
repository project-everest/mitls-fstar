...
The next highest-value implementation step is to handle receive-side alerts and handshake records in the extracted application-read loop, then audit the remaining `tls13_connection_probe.c` backend down to raw transport, HACL*/OpenSSL boundaries, and small extraction support shims.
