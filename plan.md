...
9. [in progress] Implement and verify ChaCha20-Poly1305 record protection/unprotection. `TLS13.Record.fst` now implements the Pulse-facing record-state wrapper with concrete vector-backed key/IV storage, boxed sequence/install state, verified key installation, application-data seal, and application-data open operations tied to `TLS13.Record.Spec`; the record epoch remains ghost/spec-only because it has no runtime role in AEAD protection. `extract-record-c` translates the verified wrapper to C against the trusted crypto ABI, and `test-record-bindings` links it to HACL* to check uninstalled-state rejection, application-data seal/open, and tamper rejection. C smoke coverage also combines TLSInnerPlaintext, record AAD, nonce construction, ChaCha20-Poly1305 seal/open, and tamper rejection. Broader RFC record vectors remain pending.
