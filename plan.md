...
1. The primary extracted-wrapper path runs extracted `TLS13.Connection`, extracted `TLS13.Handshake.Driver`, extracted `TLS13.Handshake.ByteDriver`, extracted `TLS13.Handshake.Transcript`, extracted `TLS13.KeySchedule`, extracted `TLS13.Record`, extracted `TLS13.Record.Framing`, and extracted `TLS13.Handshake.Framing` in the default interop test. This path proves the modeled connection/handshake sequencing, encrypted-handshake message scheduling for the scoped OpenSSL server flight, one-shot transcript hash routing for the scoped transcript shape, extracted Finished compare, key-schedule label/derivation orchestration, Finished verify-data routing, CertificateVerify input construction, Certificate leaf-DER offset parsing, CertificateVerify body length/scheme parsing, application record seal/open wrapper behavior, and no-padding application record framing before calling the remaining trusted byte backend.
2. Optional legacy diagnostic paths, enabled with `TLS13_RUN_LEGACY_INTEROP=1`, run the direct C probe and older extracted-driver route for regression comparison but are not the primary verified-client gate.
...
1. **Transcript and handshake byte driver**: the first extracted `TLS13.Handshake.ByteDriver` slice now owns the scoped encrypted server-flight schedule: it reads until a complete pending handshake message exists, enforces the exact EncryptedExtensions, Certificate, CertificateVerify, Finished order, and maps wrong type, incomplete flight, or lower-layer failure to handshake failure. `TLS13.Handshake.Transcript` now routes the scoped ClientHello || ServerHello || server-flight transcript hash through extracted code and performs the Finished verify-data equality check in extracted code. The remaining work is to move transcript byte accumulation, encrypted-handshake buffering, and Finished checkpoint materialization out of `tls13_connection_probe.c`; keep only primitive I/O, HACL*, OpenSSL X.509/signature calls, and low-level parser/serializer calls behind trusted ABIs during this step.
2. **Record framing and TLSInnerPlaintext**: route the scoped no-padding application-data framing through verified extracted code. The extracted `TLS13.Record` wrapper owns AEAD seal/open state; `TLS13.Record.Framing` now provides extracted no-padding TLSInnerPlaintext encoding for the application-data write path, no-padding content-type/payload-length decoding for the application-data read path, TLS 1.3 application-data record-header construction for writes, and record-header parsing for the read path. Padding support and advanced read/write buffering are deferred to the verified wire-code and connection-I/O milestones.
...
4. **Connection buffering and partial I/O**: implement extracted Pulse connection read/write loops that handle partial socket reads/writes, multi-record application data, handshake records appearing during application reads, close_notify, and error propagation according to `TLS13.StateMachine`.
...
6. **Interop gate update**: the extracted-wrapper path is now the default `make test-openssl-echo` client path. The direct C probe and older extracted-driver path remain optional diagnostic/regression checks behind `TLS13_RUN_LEGACY_INTEROP=1`. Remaining interop-gate work is adding fragmentation/partial-read and close_notify coverage to the wrapper path before considering public HTTPS.
...
The default `make test-openssl-echo` client path now uses extracted verified wrappers for the connection, extracted handshake driver control flow, extracted encrypted-handshake byte-driver scheduling, extracted transcript hash routing and Finished comparison, extracted key schedule, extracted record seal/open, extracted no-padding TLSInnerPlaintext encode/decode, extracted record header construction/parsing, extracted handshake header parsing, and extracted CertificateVerify input construction. The remaining trusted code in the default wrapper interop path is:

1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read/decrypt/append, transcript buffer storage, trusted Certificate/CertificateVerify body parsers, Finished verify-data storage/checkpoint fields, and application read/write loop scaffolding.
   - Replaced in the wrapper path so far: encrypted server-flight scheduling, expected handshake message order, complete-message polling, wrong-type/incomplete-flight failure routing, and the call boundary to the extracted byte driver.
   - Replaced in the wrapper path so far: scoped transcript hash routing and server Finished equality checking now run through extracted `TLS13.Handshake.Transcript`; the trusted SHA-256 primitive remains behind `TLS13.Handshake.Transcript.External`.
   - Remaining TLS-owned work to replace: transcript accumulator storage/copying, encrypted-handshake buffering, body parser implementations, Finished checkpoint materialization, partial-I/O/multi-record buffering, close_notify and fragmentation handling.
2. `tls13_wire_stubs.c`
   - Still trusted for scoped ClientHello serialization, ServerHello body parsing, legacy diagnostic paths, and non-wrapper parser/serializer call sites.
   - Replaced in the wrapper path so far: record header write/read, TLSInnerPlaintext no-padding write/read, handshake header parse, CertificateVerify input construction, Certificate leaf-DER offset parsing for the scoped generated-certificate flight, and CertificateVerify body scheme/signature-length parsing.
...
10. [in progress] Implement and verify the scoped client handshake. The pure handshake spec now models RFC 8446 server CertificateVerify input with the 64-space prefix, context string, separator, and transcript hash, and exposes Finished verification against the key-schedule verify-data function. `TLS13.Handshake.fst` now implements the Pulse-facing handshake wrapper: it carries a monotonic `TLS13.State.state_ref`, calls a lower trusted byte-operation ABI, and proves each per-flight operation advances the state token to the corresponding success phase or to `Failed` on error. The pure state machine makes successful certificate-chain validation an explicit `ValidateCertificate` event that stores the validated peer before CertificateVerify can advance. A ghost-only `TLS13.Handshake.StateDriver` verifies the controlled handshake by advancing the monotonic state token through every expected handshake event to `ApplicationData`, and `TLS13.Handshake.Driver` verifies the non-ghost control-flow spine that sequences the trusted handshake operations to either `ApplicationData` or `Failed`. `extract-handshake-c` and `extract-handshake-driver-c` now emit C for the verified wrapper and driver; `test-handshake-bindings` and `test-handshake-driver-bindings` link them against mock external ABIs to check success and failure paths. Concrete byte-level trusted-operation implementations remain pending.
11. [in progress] Implement and verify top-level connection read/write/close APIs. `TLS13.Connection.fst` now implements the top-level Pulse connection wrapper over a lower trusted connection ABI: allocation creates the monotonic state token, connect proves the controlled handshake transition to `ApplicationData` or `Failed`, write/read advance record sequence state on successful application-data operations, and close advances to `Closing` or `Failed`. `TLS13.Connection.Driver.connect_write_read_exact` verifies the end-to-end connect/write/read control-flow spine to either remain in `ApplicationData` on success or reach `Failed` on failure. Application-data and close_notify state-machine transitions advance the corresponding record-layer read/write sequence state, and a ghost-only `TLS13.Connection.StateDriver` verifies application-data send/receive echo transitions and close_notify progress over the same monotonic token. `extract-connection-c` and `extract-connection-driver-c` now emit C for the verified wrapper and driver; `test-connection-bindings` and `test-connection-driver-bindings` link them against mock external ABIs to check success and failure paths. Concrete byte-level trusted-operation implementations remain pending.
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-key-schedule-c`, `extract-record-c`, `extract-record-framing-c`, `extract-handshake-framing-c`, `extract-handshake-transcript-c`, `extract-handshake-byte-driver-c`, `extract-handshake-c`, `extract-handshake-driver-c`, `extract-connection-c`, and `extract-connection-driver-c` now translate verified Pulse/F* modules to C against explicit trusted ABIs. Binding tests link the generated C against mock or HACL-backed implementations to check success/failure paths and key/record vectors. The default OpenSSL echo smoke now uses the extracted wrapper route as the primary client path: `test/test_extracted_connection_wrapper_openssl` runs extracted `TLS13.Connection`, extracted handshake driver, extracted encrypted-handshake byte driver, extracted handshake transcript wrapper, extracted key schedule, extracted record wrapper, extracted no-padding record framing, extracted Certificate leaf-DER/CertificateVerify body parsing, and extracted CertificateVerify input framing against the scoped HACL*/wire/OpenSSL probe backend, and checks both successful multi-record echo and wrong-CA rejection. The older extracted driver route and direct C probe remain optional regression/diagnostic coverage behind `TLS13_RUN_LEGACY_INTEROP=1`. Replacing the remaining trusted byte parser, X.509, crypto, and I/O backend pieces remains pending.
...
14. [done] Add the controlled OpenSSL TLS echo interop test as the first end-to-end networking target. A local OpenSSL TLS 1.3 echo server and `openssl s_client` smoke script exist for the pinned X25519/`TLS_CHACHA20_POLY1305_SHA256` server side, covering both short and multi-record-sized payloads; a reusable C probe backend sends the scoped ClientHello built by our wire serializer, parses OpenSSL's ServerHello, Certificate, and CertificateVerify with the trusted parser stubs, derives handshake/application traffic keys with HACL* or the extracted key schedule, decrypts OpenSSL encrypted handshake records, validates the server DER leaf certificate against the generated test CA, verifies CertificateVerify using the reusable RFC 8446 input builder and verifies server Finished against the transcript, sends client Finished, sends multi-record application data, and decrypts/reassembles the exact OpenSSL echo. The direct probe uses the same per-step backend operations; the older extracted interop test routes through the verified connection and handshake drivers; and the newer extracted-wrapper interop test routes through extracted `TLS13.Connection`, extracted handshake driver, extracted `TLS13.KeySchedule`, and extracted `TLS13.Record` for application record seal/open before reaching the remaining trusted byte operations. These paths are checked to reject the OpenSSL server under a wrong generated CA. Replacing the remaining trusted byte parser, X.509, crypto, and I/O backend pieces remains tracked under the implementation work above.
...
20. [pending] Implement extracted connection I/O loops for partial reads/writes, multi-record app data, close_notify, and handshake records observed during application reads, leaving only raw socket read/write as trusted.
21. [done] Convert `make test-openssl-echo` so the extracted-wrapper path is the primary interop client and the direct C probe is only diagnostic fallback/regression coverage.
