...
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-key-schedule-c`, `extract-record-c`, `extract-record-framing-c`, `extract-handshake-framing-c`, `extract-handshake-c`, `extract-handshake-driver-c`, `extract-connection-c`, and `extract-connection-driver-c` now translate verified Pulse/F* modules to C against explicit trusted ABIs. Binding tests link the generated C against mock or HACL-backed implementations to check success/failure paths and key/record vectors. The default OpenSSL echo smoke now uses the extracted wrapper route as the primary client path: `test/test_extracted_connection_wrapper_openssl` runs extracted `TLS13.Connection`, extracted handshake driver, extracted key schedule, extracted record wrapper, extracted no-padding record framing, and extracted CertificateVerify input framing against the scoped HACL*/wire/OpenSSL probe backend, and checks both successful multi-record echo and wrong-CA rejection. The older extracted driver route and direct C probe remain optional regression/diagnostic coverage behind `TLS13_RUN_LEGACY_INTEROP=1`. Replacing the remaining trusted byte parser, X.509, crypto, and I/O backend pieces remains pending.
