...
3. The newer extracted-wrapper path, which runs extracted `TLS13.Connection`, extracted `TLS13.Handshake.Driver`, extracted `TLS13.KeySchedule`, and extracted `TLS13.Record` in the interop test. This path proves the modeled connection/handshake sequencing, key-schedule label/derivation orchestration, and application record seal/open wrapper behavior before calling the remaining trusted byte backend.
...
6. **Interop gate update**: make the extracted-wrapper path the primary `make test-openssl-echo` client path, keep the direct probe only as a diagnostic regression, and require the wrapper path to cover successful echo, wrong-CA rejection, multi-record payloads, fragmentation/partial-read cases, and close_notify behavior before considering public HTTPS.
...
12. [in progress] Configure extraction and C linking with HACL* and OpenSSL. A first `extract-smoke`/`test-extract-smoke` path verifies a small extraction-safe F* module, emits C through KaRaMeL, compiles it, and checks exported TLS version/cipher-suite constants; `extract-key-schedule-c`, `extract-record-c`, `extract-handshake-c`, `extract-handshake-driver-c`, `extract-connection-c`, and `extract-connection-driver-c` now translate verified Pulse/F* modules to C against explicit trusted ABIs. Binding tests link the generated C against mock or HACL-backed implementations to check success/failure paths and key/record vectors. The OpenSSL echo smoke now includes a newer extracted wrapper route: `test/test_extracted_connection_wrapper_openssl` runs extracted `TLS13.Connection`, extracted handshake driver, extracted key schedule, and extracted record wrapper against the scoped HACL*/wire/OpenSSL probe backend, and checks both successful multi-record echo and wrong-CA rejection. The older extracted driver route remains as regression coverage. Replacing the remaining trusted byte parser, X.509, crypto, and I/O backend pieces remains pending.
...
21. [pending] Convert `make test-openssl-echo` so the extracted-wrapper path is the primary interop client and the direct C probe is only diagnostic fallback/regression coverage.
