...
The next highest-value implementation step is to finish application-read error handling for non-close_notify alerts, then audit the remaining `tls13_connection_probe.c` backend down to raw transport, HACL*/OpenSSL boundaries, and small extraction support shims.
