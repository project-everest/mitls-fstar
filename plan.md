...
1. `tls13_connection_probe.c`
   ...
   - Replaced in the wrapper path so far: public `TLS13.Connection.connection` is now an extracted struct containing a trusted backend handle plus runtime lifecycle marker, rather than a typedef alias of the C backend connection.
