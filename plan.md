...
4. **Connection buffering and partial I/O**: the first extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, application record key installation during connect, and application write record header/inner-plaintext/seal construction. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus separate key export to connect-time key output, raw write of already-formed application records, and per-application-read hooks. Remaining work is to move each per-record read open/copy and raw record read operation fully into Pulse, handle arbitrary partial socket reads/writes, handshake records during application reads, close_notify, and error propagation according to `TLS13.StateMachine`.
...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, connect-time application-key derivation into extracted buffers, raw application-record write, per-record application-data read/open hook, and the remaining backend handle for the extracted connection.
