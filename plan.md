...
4. **Connection buffering and partial I/O**: the extracted `TLS13.Connection` application-I/O slice now owns multi-record write chunking, bounded read-record scheduling, extracted-owned application keys, extracted-owned application record-state objects, application record key installation during connect, application write record header/inner-plaintext/seal construction, and application read record-header parse/open/TLSInnerPlaintext decode/payload copy. The backend ABI has been narrowed from bulk `client_write`/`client_write_all`/`client_read`/`client_read_exact` plus separate key export to connect-time key output, raw write of already-formed application records, and raw application-record header/fragment reads. Remaining work is to move arbitrary partial socket read/write buffering into Pulse, handle handshake records during application reads, close_notify, fragmentation/padding, and error propagation according to `TLS13.StateMachine`.
...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read handoff, connect-time application-key derivation into extracted buffers, raw application-record write, raw application-record header/fragment reads, and the remaining backend handle for the extracted connection.
