...
7. [in progress] Implement trusted C parser/serializer support for the scoped TLS messages and extensions, with `.fsti` contracts and vector tests. Initial TLS record-header, handshake-header, TLSInnerPlaintext encode/decode, and scoped supported ServerHello parsing for TLS 1.3/X25519/`TLS_CHACHA20_POLY1305_SHA256` support plus malformed-input tests are present.
