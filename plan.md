...
1. `tls13_connection_probe.c`
   - Raw socket orchestration, encrypted-record read/append, transcript buffer storage, Finished verify-data storage/checkpoint fields, and application read/write loop scaffolding.
   - Replaced in the wrapper path so far: encrypted server-flight scheduling, server-handshake record open sequence/nonce/AEAD orchestration, expected handshake message order, complete-message polling, wrong-type/incomplete-flight failure routing, and the call boundary to the extracted byte driver.
...
2. `tls13_wire_stubs.c`
   ...
   - Replaced in the wrapper path so far: record header write/read, TLSInnerPlaintext no-padding write/read including encrypted-handshake inner plaintext, handshake header parse, CertificateVerify input construction, Certificate leaf-DER offset parsing for the scoped generated-certificate flight, and CertificateVerify body scheme/signature-length parsing.
