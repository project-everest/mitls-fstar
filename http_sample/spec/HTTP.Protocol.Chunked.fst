module HTTP.Protocol.Chunked

(**
  HTTP/1.1 response download with *chunked* transfer-encoding, as
  state-machine type-class instances over the hand-written wire union
  `HTTP.Wire.Chunked.http_message`.

  Direction modeled: server (response sender) → client (receiver).  The server
  serves a file body as a sequence of chunks, each carrying up to
  `http_block_size` payload bytes; the client reassembles the body and completes
  on the zero-length final chunk (`0\r\n\r\n`).

  ── The file-transfer profile (how HTTP differs from TFTP/YMODEM) ────────────

    * ORDERING IS POSITIONAL, not indexed.  A chunk carries no block number on
      the wire; order is its position in the stream, so
      `ft_classify (Msg_chunk d) = FT_Data None d` (as in FTP block mode over
      TCP).  `ft_law_data_wire`'s index obligation is therefore vacuous (`None`).

    * UNBOUNDED, TRANSPORT-RELIABLE WINDOW.  HTTP runs over TCP, which provides
      ordering and flow control, so the application does no windowing:
      `ft_window = None`.

    * NO BODY-LEVEL ACKS.  HTTP has no per-chunk acknowledgment, so the wire
      union has no ack message; `FT_Ack` is never classified and
      `ft_law_ack_wire` is vacuous.  The acked prefix stays 0.

    * NO TIMEOUTS / RETRANSMISSION.  TCP handles loss recovery, so there is no
      retransmission timeout: `ft_is_timeout` is always false and
      `ft_law_timeout` is vacuous.

    * UNPADDED.  A chunk carries its exact payload and the zero-length final
      chunk terminates, so reassembly equals the body exactly
      (`reassembly_exact` degenerates to a full equality).

    * THE START IS A LOCAL EVENT.  The request line and the response head are
      produced/consumed by the (verified) codec in the impl harness, which then
      drives the modeled transfer from a local `Server_start` event.  Hence
      request/response classify as `FT_Other` and `ft_law_request_wire` is
      vacuous.
**)
...
(* Maximum chunk payload (the 4-hex-digit chunk-size field caps a chunk at
   65535 bytes, matching `HTTP.Wire.Chunked.chunk_payload`). *)
let http_block_size : n:nat{n >= 1} = 65535

(* A "plan" is the body pre-framed into chunks, each between 1 and
   `http_block_size` bytes (a non-empty chunk carries body data; the zero-length
   chunk is reserved as the framing terminator). *)
let rec plan_wf (l:list TCP.bytes) : Tot prop (decreases l) =
  match l with
  | [] -> True
  | x :: tl -> 1 <= Seq.length x /\ Seq.length x <= http_block_size /\ plan_wf tl
...
type http_server_state = {
  ...
  hss_sent     : list TCP.bytes;      // chunk payloads already sent, in order
  hss_pending  : list TCP.bytes;      // chunk payloads not yet sent
...
(* Local events driving the server:
     * Server_start   — the request/response head has been processed (by the
       verified codec in the harness): the served file name and its pre-framed
       chunks;
     * Server_send    — emit the next chunk;
     * Server_complete— all chunks emitted: send the zero-length final chunk and
       complete;
...
(* The raw reassembly of every chunk (sent and pending): the whole body.
   Invariant under Server_send. *)
...
(* Abstract file-transfer view of a server state.  Positional/unbounded/unacked:
   `ftv_acked` is pinned to 0 (there are no acks). *)
...
(* The server emits the next chunk: it moves the head of the pending list onto
   the sent list.  Positional: no wire block number.  Unbounded window: no
   in-flight bound. *)
let http_server_send (s0 s1:http_server_state) (d:chunk_payload) : prop =
...
let http_server_step
  ...
  | SM.LocalEvent Server_send ->
    (exists (d:chunk_payload).
       http_server_send s0 s1 d /\ out.SM.so_wire_outputs == [Msg_chunk d]) /\
    out.SM.so_local_outputs == []
  | SM.LocalEvent Server_complete ->
    ...
    (* the zero-length final chunk terminates the body on the wire *)
    out.SM.so_wire_outputs == [Msg_chunk (Seq.empty <: chunk_payload)] /\
    out.SM.so_local_outputs == []
...
(* ───────────────────────────────────────────────────────────────────────────
   ...
   from the chunk payloads and completes on the zero-length final chunk.
   ─────────────────────────────────────────────────────────────────────────── *)

noeq
type http_client_state = {
  hcs_filename : option TCP.bytes;   // file the client is receiving
  hcs_received : list TCP.bytes;     // chunk payloads received, in order
...
type http_client_local =
  | Client_start : filename:TCP.bytes -> http_client_local

(* The exact file the client reconstitutes: the concatenation of the received
   chunk payloads (unpadded — the zero-length final chunk ends it). *)
...
let http_client_step
  ...
  | SM.WireEvent m ->
    (match m with
     | Msg_chunk d ->
       (* receive a chunk: append it; a zero-length chunk ends the transfer *)
       Some? s0.hcs_filename /\
       s0.hcs_status == FT.FT_InProgress /\
       s1.hcs_filename == s0.hcs_filename /\
       (if Seq.length (d <: TCP.bytes) = 0
        then s1.hcs_received == s0.hcs_received /\ s1.hcs_status == FT.FT_Completed
        else s1.hcs_received == L.append s0.hcs_received [(d <: TCP.bytes)] /\
             s1.hcs_status == FT.FT_InProgress) /\
       out.SM.so_wire_outputs == [] /\
       out.SM.so_local_outputs == []
     | _ -> False)
...
(* A non-empty chunk is a positional data block (`FT_Data None`); the zero-length
   final chunk is the framing terminator (classified FT_Other, like the
   request/response heads, which are impl glue since the transfer starts from a
   local event). *)
let http_classify (m:http_message) : FT.ft_packet =
  match m with
  | Msg_chunk d -> if Seq.length (d <: TCP.bytes) = 0
                   then FT.FT_Other
                   else FT.FT_Data None (d <: TCP.bytes)
  | _           -> FT.FT_Other
...
=
  ...
  | _ ->
    (* Server_start / Server_abort emit no wire output; Server_complete emits only
       ...
       precondition FT_Data?(classify msg) is unsatisfiable in every other case. *)
    ()
...
(* Capstone: the generic reconstitution theorem for the chunked HTTP server.  In
   any reachable state serving a file, the raw reassembly agrees with the body on
   their common prefix, and on completion reconstitutes the exact body (a full
   equality, since HTTP chunked is unpadded). *)
