...
$KRML -tmpdir _extract -skip-compilation -warn-error -2-9-17 \
  ...
  -no-prefix HTTP.Impl.Codec.RequestLine \
  -no-prefix HTTP.Impl.Loop.Request \
  -no-prefix HTTP.Impl.Codec.RequestPost \
  -no-prefix HTTP.Impl.Loop.RequestPost \
  $KRMLS
echo "=== generated ==="
