...
MODS="HTTP.Impl.Codec.Chunked HTTP.Impl.Server.Loop HTTP.Impl.Client.Loop"
...
$KRML -tmpdir _extract -skip-compilation -warn-error -2-9-17 \
  -bundle 'HTTP.Impl.Server.Loop+HTTP.Impl.Client.Loop+HTTP.Impl.Codec.Chunked=[rename=HTTP_Verified]' \
