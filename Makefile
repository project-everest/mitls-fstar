...
IMPL_FILES = \
  ...
  src/impl/TLS13.Handshake.Framing.fsti \
  src/impl/TLS13.Handshake.Framing.fst \
  src/impl/TLS13.Handshake.FlightState.fsti \
  src/impl/TLS13.Handshake.FlightState.fst \
  src/impl/TLS13.Handshake.Transcript.External.fsti \
  src/impl/TLS13.Handshake.Transcript.fsti \
  src/impl/TLS13.Handshake.Transcript.fst \
  src/impl/TLS13.Handshake.ByteDriver.External.fsti \
  src/impl/TLS13.Handshake.ByteDriver.fst \
...
EXTRACT_HANDSHAKE_FRAMING_DIR  = $(EXTRACT_DIR)/handshake-framing
EXTRACT_HANDSHAKE_FRAMING_KRML = $(EXTRACT_HANDSHAKE_FRAMING_DIR)/TLS13_Handshake_Framing.krml
EXTRACT_HANDSHAKE_FRAMING_C    = $(EXTRACT_HANDSHAKE_FRAMING_DIR)/TLS13_Handshake_Framing.c
EXTRACT_HANDSHAKE_FRAMING_H    = $(EXTRACT_HANDSHAKE_FRAMING_DIR)/TLS13_Handshake_Framing.h
EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR  = $(EXTRACT_DIR)/handshake-flight-state
EXTRACT_HANDSHAKE_FLIGHT_STATE_KRML = $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR)/TLS13_Handshake_FlightState.krml
EXTRACT_HANDSHAKE_FLIGHT_STATE_C    = $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR)/TLS13_Handshake_FlightState.c
EXTRACT_HANDSHAKE_FLIGHT_STATE_H    = $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR)/TLS13_Handshake_FlightState.h
EXTRACT_HANDSHAKE_TRANSCRIPT_DIR  = $(EXTRACT_DIR)/handshake-transcript
EXTRACT_HANDSHAKE_TRANSCRIPT_KRML = $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR)/TLS13_Handshake_Transcript.krml
EXTRACT_HANDSHAKE_TRANSCRIPT_C    = $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR)/TLS13_Handshake_Transcript.c
EXTRACT_HANDSHAKE_TRANSCRIPT_H    = $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR)/TLS13_Handshake_Transcript.h
EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR  = $(EXTRACT_DIR)/handshake-byte-driver
EXTRACT_HANDSHAKE_BYTE_DRIVER_KRML = $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR)/TLS13_Handshake_ByteDriver.krml
EXTRACT_HANDSHAKE_BYTE_DRIVER_C    = $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR)/TLS13_Handshake_ByteDriver.c
EXTRACT_HANDSHAKE_BYTE_DRIVER_H    = $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR)/TLS13_Handshake_ByteDriver.h
...
$(EXTRACT_HANDSHAKE_FRAMING_DIR):
	mkdir -p $@

$(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR):
	mkdir -p $@

$(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR):
	mkdir -p $@

$(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR):
	mkdir -p $@
...
$(EXTRACT_HANDSHAKE_FRAMING_KRML): src/impl/TLS13.Handshake.Framing.fst verify | $(EXTRACT_HANDSHAKE_FRAMING_DIR)
	$(FSTAR_EXE) --cache_checked_modules --cache_dir $(CACHE_DIR) --odir $(OUTPUT_DIR) \
	  --warn_error -321 --report_assumes warn \
	  --already_cached 'Prims,FStar,Pulse,PulseCore -TLS13' \
	  $(INCLUDES) --codegen krml --extract_module TLS13.Handshake.Framing \
	  --krmloutput $@ $<

extract-handshake-framing-krml: $(EXTRACT_HANDSHAKE_FRAMING_KRML)

$(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_FRAMING_H): $(EXTRACT_HANDSHAKE_FRAMING_KRML) | $(EXTRACT_HANDSHAKE_FRAMING_DIR)
	$(KRML_EXE) -skip-compilation -skip-makefiles -warn-error -2 \
	  -tmpdir $(EXTRACT_HANDSHAKE_FRAMING_DIR) $(EXTRACT_HANDSHAKE_FRAMING_KRML)

extract-handshake-framing-c: $(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_FRAMING_H)

$(EXTRACT_HANDSHAKE_FLIGHT_STATE_KRML): src/impl/TLS13.Handshake.FlightState.fst verify | $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR)
	$(FSTAR_EXE) --cache_checked_modules --cache_dir $(CACHE_DIR) --odir $(OUTPUT_DIR) \
	  --warn_error -321 --report_assumes warn \
	  --already_cached 'Prims,FStar,Pulse,PulseCore -TLS13' \
	  $(INCLUDES) --codegen krml --extract_module TLS13.Handshake.FlightState \
	  --krmloutput $@ $<

extract-handshake-flight-state-krml: $(EXTRACT_HANDSHAKE_FLIGHT_STATE_KRML)

$(EXTRACT_HANDSHAKE_FLIGHT_STATE_C) $(EXTRACT_HANDSHAKE_FLIGHT_STATE_H): $(EXTRACT_HANDSHAKE_FLIGHT_STATE_KRML) $(EXTRACT_RECORD_H) $(EXTRACT_RECORD_FRAMING_H) $(EXTRACT_KEY_SCHEDULE_H) $(EXTRACT_HANDSHAKE_TRANSCRIPT_H) c_stubs/tls13_record_for_flight_state.h c_stubs/tls13_crypto_external.h c_stubs/tls13_handshake_transcript_external.h | $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR)
	$(KRML_EXE) -skip-compilation -skip-makefiles -warn-error -2 \
	  -add-include '"tls13_record_for_flight_state.h"' \
	  -tmpdir $(EXTRACT_HANDSHAKE_FLIGHT_STATE_DIR) $(EXTRACT_HANDSHAKE_FLIGHT_STATE_KRML)

extract-handshake-flight-state-c: $(EXTRACT_HANDSHAKE_FLIGHT_STATE_C) $(EXTRACT_HANDSHAKE_FLIGHT_STATE_H)

$(EXTRACT_HANDSHAKE_TRANSCRIPT_KRML): src/impl/TLS13.Handshake.Transcript.fst verify | $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR)
	$(FSTAR_EXE) --cache_checked_modules --cache_dir $(CACHE_DIR) --odir $(OUTPUT_DIR) \
	  --warn_error -321 --report_assumes warn \
	  --already_cached 'Prims,FStar,Pulse,PulseCore -TLS13' \
	  $(INCLUDES) --codegen krml --extract_module TLS13.Handshake.Transcript \
	  --krmloutput $@ $<

extract-handshake-transcript-krml: $(EXTRACT_HANDSHAKE_TRANSCRIPT_KRML)

$(EXTRACT_HANDSHAKE_TRANSCRIPT_C) $(EXTRACT_HANDSHAKE_TRANSCRIPT_H): $(EXTRACT_HANDSHAKE_TRANSCRIPT_KRML) c_stubs/tls13_handshake_transcript_external.h | $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR)
	$(KRML_EXE) -skip-compilation -skip-makefiles -warn-error -2 \
	  -add-include '"tls13_handshake_transcript_external.h"' \
	  -tmpdir $(EXTRACT_HANDSHAKE_TRANSCRIPT_DIR) $(EXTRACT_HANDSHAKE_TRANSCRIPT_KRML)

extract-handshake-transcript-c: $(EXTRACT_HANDSHAKE_TRANSCRIPT_C) $(EXTRACT_HANDSHAKE_TRANSCRIPT_H)

$(EXTRACT_HANDSHAKE_BYTE_DRIVER_KRML): src/impl/TLS13.Handshake.ByteDriver.fst verify | $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR)
	$(FSTAR_EXE) --cache_checked_modules --cache_dir $(CACHE_DIR) --odir $(OUTPUT_DIR) \
	  --warn_error -321 --report_assumes warn \
	  --already_cached 'Prims,FStar,Pulse,PulseCore -TLS13' \
	  $(INCLUDES) --codegen krml --extract_module TLS13.Handshake.ByteDriver \
	  --krmloutput $@ $<

extract-handshake-byte-driver-krml: $(EXTRACT_HANDSHAKE_BYTE_DRIVER_KRML)

$(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_H): $(EXTRACT_HANDSHAKE_BYTE_DRIVER_KRML) c_stubs/tls13_handshake_byte_driver_external.h | $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR)
	$(KRML_EXE) -skip-compilation -skip-makefiles -warn-error -2 \
	  -add-include '"tls13_handshake_byte_driver_external.h"' \
	  -tmpdir $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_KRML)

extract-handshake-byte-driver-c: $(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_H)
...
test/test_connection_bindings: test/test_connection_bindings.c $(EXTRACT_CONNECTION_C) $(EXTRACT_CONNECTION_H) $(EXTRACT_RECORD_C) $(EXTRACT_RECORD_H) $(EXTRACT_RECORD_FRAMING_C) $(EXTRACT_RECORD_FRAMING_H) c_stubs/tls13_connection_external_layer.h c_stubs/tls13_crypto_external.c c_stubs/tls13_pulse_shims.c $(HACL_WRAPPER_SOURCES) | check-deps
	$(CC) -Wall -Wextra -Wno-deprecated-declarations \
	  -ffunction-sections -fdata-sections \
	  -I $(EXTRACT_CONNECTION_DIR) -I $(EXTRACT_RECORD_DIR) -I $(EXTRACT_RECORD_FRAMING_DIR) -I c_stubs -I $(KRML_HOME)/include -I $(KRML_HOME)/krmllib/dist/minimal \
	  -I $(HACL_DIR) -I $(HACL_DIR)/internal -I $(HACL_KI) -I $(HACL_KL) \
	  $(EXTRACT_CONNECTION_C) $(EXTRACT_RECORD_C) $(EXTRACT_RECORD_FRAMING_C) c_stubs/tls13_crypto_external.c c_stubs/tls13_pulse_shims.c test/test_connection_bindings.c $(HACL_WRAPPER_SOURCES) \
	  -Wl,--gc-sections -o $@
...
test/test_handshake_bindings: test/test_handshake_bindings.c $(EXTRACT_HANDSHAKE_C) $(EXTRACT_HANDSHAKE_H) $(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_FRAMING_H) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_H) $(EXTRACT_RECORD_FRAMING_C) $(EXTRACT_RECORD_FRAMING_H) c_stubs/tls13_handshake_external_layer.h c_stubs/tls13_handshake_byte_driver_external.h
	$(CC) -Wall -Wextra \
	  -I $(EXTRACT_HANDSHAKE_DIR) -I $(EXTRACT_HANDSHAKE_FRAMING_DIR) -I $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR) -I $(EXTRACT_RECORD_FRAMING_DIR) -I c_stubs -I $(KRML_HOME)/include -I $(KRML_HOME)/krmllib/dist/minimal \
	  $(EXTRACT_HANDSHAKE_C) $(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_RECORD_FRAMING_C) c_stubs/tls13_pulse_shims.c test/test_handshake_bindings.c \
	  -o $@
