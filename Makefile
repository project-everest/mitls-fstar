...
.PHONY: all verify test extract-smoke extract-connection-driver-krml extract-connection-driver-c extract-connection-krml extract-connection-c extract-handshake-driver-krml extract-handshake-driver-c extract-handshake-krml extract-handshake-c extract-handshake-framing-krml extract-handshake-framing-c extract-handshake-byte-driver-krml extract-handshake-byte-driver-c extract-key-schedule-krml extract-key-schedule-c extract-record-krml extract-record-c extract-record-framing-krml extract-record-framing-c test-extract-smoke test-connection-driver-bindings test-connection-bindings test-handshake-driver-bindings test-handshake-bindings test-key-schedule-bindings test-record-bindings check-c-stubs test-hacl-stubs test-openssl-stubs test-wire-stubs test-record-stubs test-io-stubs test-openssl-echo check-toolchain check-deps clean
...
test/test_extracted_connection_wrapper_openssl: $(CONNECTION_PROBE_SOURCES) test/test_extracted_connection_wrapper_openssl.c $(EXTRACT_CONNECTION_C) $(EXTRACT_CONNECTION_H) $(EXTRACT_HANDSHAKE_DRIVER_C) $(EXTRACT_HANDSHAKE_DRIVER_H) $(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_FRAMING_H) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_H) $(EXTRACT_KEY_SCHEDULE_C) $(EXTRACT_KEY_SCHEDULE_H) $(EXTRACT_RECORD_C) $(EXTRACT_RECORD_H) $(EXTRACT_RECORD_FRAMING_C) $(EXTRACT_RECORD_FRAMING_H) c_stubs/tls13_connection_external_layer.h c_stubs/tls13_crypto_external.c c_stubs/tls13_pulse_shims.c c_stubs/tls13_handshake_byte_driver_external.h | check-deps
	$(CC) -Wall -Wextra -Wno-deprecated-declarations \
	  ...
	  -I $(EXTRACT_CONNECTION_DIR) -I $(EXTRACT_HANDSHAKE_DRIVER_DIR) -I $(EXTRACT_HANDSHAKE_FRAMING_DIR) -I $(EXTRACT_HANDSHAKE_BYTE_DRIVER_DIR) -I $(EXTRACT_KEY_SCHEDULE_DIR) -I $(EXTRACT_RECORD_DIR) -I $(EXTRACT_RECORD_FRAMING_DIR) \
	  -I c_stubs -I $(KRML_HOME)/include -I $(KRML_HOME)/krmllib/dist/minimal \
	  -I $(HACL_DIR) -I $(HACL_DIR)/internal -I $(HACL_KI) -I $(HACL_KL) \
	  $(EXTRACT_CONNECTION_C) $(EXTRACT_HANDSHAKE_DRIVER_C) $(EXTRACT_HANDSHAKE_FRAMING_C) $(EXTRACT_HANDSHAKE_BYTE_DRIVER_C) $(EXTRACT_KEY_SCHEDULE_C) $(EXTRACT_RECORD_C) $(EXTRACT_RECORD_FRAMING_C) \
