...
(**
  Server-side semantic replay slice for receiving the protected
  ClientFinished record.

  This is deliberately lower-level than the final projection witness: it starts
  from the already-verified ordered post-ServerHello server replay and splits it
  at the point just after the server has sent its protected flight.  The suffix
  is exactly:

    server application write install; receive ClientFinished;
    server application read install; verify ClientFinished.

  The first two post-ServerHello handshake installs remain in the server-flight
  prefix where they are legal; unlike the stale staged-v2 shape, no handshake
  install is delayed until the ClientFinished boundary.
**)
noextract
let server_client_finished_received_decode_suffix_replay_slice
...
val lemma_server_client_finished_received_decode_suffix_replay_slice_from_ordered_post_server_hello
  (server:CS.connection_state)
  : Lemma
