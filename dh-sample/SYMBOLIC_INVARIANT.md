...
| `ActDeliver idx Resp` on Msg1 (responder sends Msg2) | `Event tag_responder_respond`, `RandGen` (nonce), `MsgSent (SMsg2 me share sig)` | event shape + `p == resp_dy_principal`; `is_publishable` of the SIGNATURE, hence `dh_sign_pred`'s responder disjunct | `lemma_step_trace_invariant_resp_msg1` → `lemma_respond_trace_invariant`: `lemma_responder_respond_event_pred` (called with `mep = resp_dy_principal`); `lemma_net_entry_share_publishable` (received Msg1 share, honest-or-injected) + `lemma_share_publishable` (own share) build the transcript's publishability; `lemma_sig_term_publishable` discharges the signature from the JUST-triggered `tag_responder_respond` event over that EXACT transcript |
