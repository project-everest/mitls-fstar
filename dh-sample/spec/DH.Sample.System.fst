...
(**
  ...
  Completion still has an ideal-signature boundary: a Msg2 accepted by the
  initiator must be a packet actually sent by the responder, and a Msg3 accepted
  by the responder must be one actually sent by the initiator.  The toy digest
  is deliberately not used as an unforgeability argument.
*)
